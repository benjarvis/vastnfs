			       ...
			       size_t count, size_t seek)
{
	...
	BUG_ON(seek > count);
	total = count - seek;
	iov_iter_init(&iter, msg->msg_iov, msg->msg_iovlen, total, seek);
	BUG_ON(msg->msg_iovlen == 0);

	kvec = (struct kvec *)iter.iov;
	BUG_ON(iter.iov_offset >= kvec->iov_len);
	msg->msg_iov = (struct iovec *)iter.iov;
	kvec->iov_base += iter.iov_offset;
	kvec->iov_len -= iter.iov_offset;

	scan = total;
	for (i = 0; i < iter.nr_segs; i++) {
		if (kvec[i].iov_len <= scan) {
			scan -= kvec[i].iov_len;
		} else {
			kvec[i].iov_len = scan;
			iter.nr_segs = i + 1;
	...
	msg->msg_iovlen = iter.nr_segs;
	return kernel_sendmsg(sock, msg, kvec, iter.nr_segs, total);
}

static int xprt_send_kvec(struct socket *sock, struct msghdr *msg,
			  struct kvec *vec, size_t seek)
{
	struct iovec iovec = {vec->iov_base, vec->iov_len};
	msg->msg_iov = &iovec;
	msg->msg_iovlen = 1;
	return xprt_sendmsg(sock, msg, vec->iov_len, seek);
}
			      ...
			      struct xdr_buf *xdr, size_t base)
{
	...
	memcpy(xdr->tmp_iovec, xdr->iovec, xdr_buf_pagecount(xdr)
	       * sizeof(xdr->iovec[0]));

	msg->msg_iovlen = xdr_buf_pagecount(xdr);
	msg->msg_iov = xdr->tmp_iovec;

	return xprt_sendmsg(sock, msg, xdr->page_len + xdr->page_base,
			    base + xdr->page_base);
}
				 ...
				 size_t base)
{
	struct iovec iov[2] = {
		...
		[1] = {
			.iov_base       = vec->iov_base,
			.iov_len       = vec->iov_len,
		},
	};
	size_t count = iov[0].iov_len + iov[1].iov_len;
	msg->msg_iov = &iov[0];
	msg->msg_iovlen = 2;

	return xprt_sendmsg(sock, msg, count, base);
}
