 ...
 */
static ssize_t svc_recvfrom(struct svc_rqst *rqstp, struct iovec *iov,
			    unsigned int nr, size_t buflen, unsigned int base)
{
	...
	iov_iter_init(&iter, iov, nr, buflen, base);
	...
	len = kernel_recvmsg(svsk->sk_sock, &msg, (struct kvec *)iter.iov, iter.nr_segs, buflen, MSG_DONTWAIT);
 ...
 */
static void svc_data_ready(struct sock *sk, int len)
{
 ...
 */
static void svc_tcp_listen_data_ready(struct sock *sk, int len)
{
	struct svc_sock	*svsk = (struct svc_sock *)sk->sk_user_data;
...
static int copy_pages_to_kvecs(struct iovec *vec, struct page **pages, int len)
{
	int i = 0;
	int t = 0;
