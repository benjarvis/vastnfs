...
xs_alloc_sparse_pages(struct xdr_buf *buf, size_t want, gfp_t gfp)
{
	...
	for (i = 0; i < n; i++) {
		...
		buf->iovec[i].iov_base = page_address(buf->pages[i]);
	}
	return want;
}

static ssize_t
xs_sock_recvmsg(struct socket *sock, struct msghdr *msg, int flags, size_t count, size_t seek)
{
	ssize_t ret, max_count = 0, i, xfer;
	struct kvec *kvec;
	struct iov_iter iter;
	struct iovec *orig = msg->msg_iov;
	int orig_len = msg->msg_iovlen;

	BUG_ON(seek > count);
	BUG_ON(orig_len == 0);

	xfer = count - seek;
	iov_iter_init(&iter, orig, orig_len, xfer, seek);
	BUG_ON((long)iter.nr_segs < 0);

	kvec = (struct kvec *)iter.iov;
	kvec->iov_base += iter.iov_offset;
	kvec->iov_len -= iter.iov_offset;

	for (i = 0; i < iter.nr_segs; i++)
		max_count += kvec[i].iov_len;
	if (xfer > max_count && !(flags & MSG_TRUNC))
		xfer = max_count;

	ret = kernel_recvmsg(sock, msg, kvec, iter.nr_segs, xfer, flags);
...
{
	struct iovec iovec = {kvec->iov_base, kvec->iov_len};
	msg->msg_iov = &iovec;
	msg->msg_iovlen = 1;
	return xs_sock_recvmsg(sock, msg, flags, count, seek);
}

static ssize_t
xs_read_bvec(struct socket *sock, struct msghdr *msg, int flags,
	    struct iovec *iovec, struct iovec *tmp_iovec,
	    unsigned long nr, size_t count, size_t seek)
{
	memcpy(tmp_iovec, iovec, nr * sizeof(iovec[0]));
	msg->msg_iov = tmp_iovec;
	msg->msg_iovlen = nr;
	return xs_sock_recvmsg(sock, msg, flags, count, seek);
}
...
static void
xs_flush_bvec(const struct iovec *iovec, size_t count, size_t seek)
{
	struct iov_iter iter;
	unsigned int len;
	...
	iov_iter_init(&iter, iovec, count - seek, count - seek, seek);

	while (iter.nr_segs > 0 && iter.count > 0) {
		const bool is_null = iter.iov->iov_base == NULL;
		const size_t size = min_t(u32,
			iter.iov->iov_len - iter.iov_offset, iter.count);
		void *ptr = iter.iov->iov_base + iter.iov_offset;
		size_t remaining = size;

		while (!is_null && remaining > 0) {
			flush_dcache_page(virt_to_page(ptr));
			len = min_t(u32, PAGE_SIZE - offset_in_page(ptr), remaining);
			ptr += len;
			remaining -= len;
		}
		...
		struct xdr_buf *buf, size_t count, size_t seek, size_t *read)
{
	...
	want = xs_alloc_sparse_pages(buf,
			...
			GFP_KERNEL);
	if (seek < want) {
		ret = xs_read_bvec(sock, msg, flags, buf->iovec,
				   buf->tmp_iovec,
		...
		if (ret <= 0)
			goto sock_err;
		xs_flush_bvec(buf->iovec, ret, seek + buf->page_base);
		ret -= buf->page_base;
		offset += ret;
		if (offset == count || msg->msg_flags & (MSG_EOR|MSG_TRUNC))
			goto out;
...
static unsigned int xs_poll_socket(struct sock_xprt *transport)
{
...
{
	unsigned int events = xs_poll_socket(transport);

	return (events & (POLLIN | POLLRDNORM)) && !(events & POLLRDHUP);
}
...
static int xs_bind(struct sock_xprt *transport, struct socket *sock)
{
	...
	if (transport->xprt.localport_len == transport->xprt.addrlen)
		memcpy(&myaddr, &transport->xprt.localport, transport->xprt.addrlen);
