...
enum {
	...
	Opt_localports, Opt_remoteports, Opt_noidlexprt,
	Opt_remoteports_offset, Opt_localports_failover,
	Opt_nconnect,
	Opt_lookupcache,
	Opt_fscache_uniq,
	Opt_local_lock,
...
static const match_table_t nfs_mount_option_tokens = {
	...
	{ Opt_forcerdirplus, "forcerdirplus" },
	...
	{ Opt_noidlexprt, "noidlexprt" },
	{ Opt_localports_failover, "localports_failover" },
	...
	{ Opt_localports, "localports=%s" },
	{ Opt_remoteports, "remoteports=%s" },
	{ Opt_remoteports_offset, "remoteports_offset=%s" },
...
static void nfs_free_parsed_mount_data(struct nfs_parsed_mount_data *data)
{
	if (data) {
		...
		if (data->local_ports)
			vfree(data->local_ports);
		if (data->remote_ports)
			vfree(data->remote_ports);
...
static int nfs_parse_port_group(char *string, struct nfs_parsed_mount_data *mnt,
				struct rpc_portgroup **pg_out, const char *type)
{
	...
	while (string_scan != NULL) {
		...
		if (!prev_range) {
			...
			pg->lens[pg->nr] = rpc_pton(mnt->net, p, strlen(p),
						    (struct sockaddr *)
						    &pg->addrs[pg->nr],
			...
			len = rpc_pton(mnt->net, p, strlen(p),
			...
			while (true) {
				...
				pg->addrs[pg->nr] = pg->addrs[pg->nr - 1];
				pg->lens[pg->nr] = pg->lens[pg->nr - 1];

				switch (addr.ss_family) {
				case AF_INET: {
					struct sockaddr_in *sin1 =
						(struct sockaddr_in *)&pg->addrs[pg->nr];
					sin1->sin_addr.s_addr =
						htonl(ntohl(
						sin1->sin_addr.s_addr) + 1);
					break;
				}
				case AF_INET6:
					return -ENOTSUPP;
				   ...
				   struct nfs_parsed_mount_data *mnt)
{
	...
	while ((p = strsep(&raw, ",")) != NULL) {
		...
		case Opt_forcerdirplus:
			mnt->flags |= NFS_MOUNT_FORCE_READDIRPLUS;
			break;
		case Opt_noforcerdirplus:
			mnt->flags &= ~NFS_MOUNT_FORCE_READDIRPLUS;
			break;
		case Opt_sharecache:
		...
		case Opt_nconnect:
			...
			if (!strcmp(string, "mesh")) {
				mnt->nfs_server.nconnect_mesh = true;
			} else {
		...
		case Opt_remoteports_offset:
			...
			if (string == NULL) {
				...
				if (!nfs_get_option_ul_bound(args, &option, 1, 0x100)) {
					mnt->nfs_server.remoteports_offset = option;
					mnt->nfs_server.remoteports_offset_provided = true;
				} else {
					kfree(string);
					goto out_nomem;
				}
		...
		case Opt_localports_failover:
			mnt->local_ports_failover = true;
			break;
		case Opt_localports:
			string = match_strdup(args);
			if (string == NULL)
				goto out_nomem;
			rc = nfs_parse_port_group(string, mnt, &mnt->local_ports, "local");
			kfree(string);
		...
		case Opt_remoteports:
			...
			if (string == NULL)
				goto out_nomem;
			rc = nfs_parse_port_group(string, mnt, &mnt->remote_ports, "remote");
			kfree(string);
	...
	if (mnt->nfs_server.nconnect_mesh) {
		int remote_ports = 1;
		int local_ports = 1;

		if (mnt->local_ports)
			local_ports = mnt->local_ports->nr;
		if (mnt->remote_ports)
			remote_ports = mnt->remote_ports->nr;
		mnt->nfs_server.nconnect = remote_ports * local_ports;
	}
...
out_portgroup_too_large:
	printk(KERN_INFO "NFS: portgroup is too large\n");
	return 0;
}
