		 ...
		 unsigned short multipath_hash)
{
	int res;
	do {
		res = rpc_call_sync_multipath(clnt, msg, flags, multipath_hash);
		if (res != -EJUKEBOX)
			break;
...
#define rpc_call_sync(clnt, msg, flags)	nfs3_rpc_wrapper(clnt, msg, flags, 0)
#define rpc_call_sync_multipath(clnt, msg, flags, hash)	\
	nfs3_rpc_wrapper(clnt, msg, flags, hash)
		...
		struct nfs_fattr *fattr, struct nfs4_label *label)
{
	...
	status = rpc_call_sync_multipath(server->client, &msg, 0,
		 ...
		 struct nfs4_label *label)
{
	...
	nfs_fattr_init(fattr);
	status = rpc_call_sync_multipath(NFS_CLIENT(dir), &msg, 0, NFS_FH_HASH_INODE(dir));
...
nfs3_proc_link(struct inode *inode, struct inode *dir, const struct qstr *name)
{
	...
	status = rpc_call_sync_multipath(NFS_CLIENT(inode), &msg, 0, NFS_FH_HASH_INODE(dir));
	nfs_post_op_update_inode(dir, res.dir_attr);
