...
FLAVOR ?= $(variant)
OFA_ORIG = $(shell ( test -d $(OFA_DIR)/$$(uname -m)/$(KVER) && echo $(OFA_DIR)/$$(uname -m)/$(KVER) ) || ( test -d $(OFA_DIR)/$(KVER) && echo $(OFA_DIR)/$(KVER) ) || ( test -d $(OFA_DIR)/$(FLAVOR) && echo $(OFA_DIR)/$(FLAVOR) ) || ( test -d /var/lib/dkms/mlnx-ofed-kernel/ && ls -d /var/lib/dkms/mlnx-ofed-kernel/*/build ) || ( echo $(OFA_DIR) ))
...
CONFIG_NFSD_NEW_FAULT_INJECTION ?= n
CONFIG_NFS_MOUNT_TRACES ?= n
CONFIG_NFS_BUFFERED_INTERNAL_WRITEBACK_ENABLED := n

ifeq ($(CONFIG_NFSD_NEW_FAULT_INJECTION),y)
$(info NFSD fault injection support: enabled)
EXTRA_CFLAGS += -DCONFIG_NFSD_NEW_FAULT_INJECTION=$(CONFIG_NFSD_NEW_FAULT_INJECTION)
else
$(info NFSD fault injection support: disabled)
endif
ifeq ($(CONFIG_NFS_MOUNT_TRACES),y)
$(info Mounts tracing support: enabled)
EXTRA_CFLAGS += -DCONFIG_NFS_MOUNT_TRACES=$(CONFIG_NFS_MOUNT_TRACES)
else
$(info Mounts tracing support: disabled)
endif
ifeq ($(CONFIG_NFS_BUFFERED_INTERNAL_WRITEBACK_ENABLED),y)
$(info Buffered internal writeback default: enabled)
EXTRA_CFLAGS += -DCONFIG_NFS_BUFFERED_INTERNAL_WRITEBACK_ENABLED=$(CONFIG_NFS_BUFFERED_INTERNAL_WRITEBACK_ENABLED)
else
$(info Buffered internal writeback default: disabled)
endif
ifeq ($(CONFIG_NFS_CRASH_ON_WARN),y)
EXTRA_CFLAGS += -DCONFIG_NFS_CRASH_ON_WARN=$(CONFIG_NFS_CRASH_ON_WARN)
endif


CONFIG_SUNRPC_XPRT_RDMA ?= m

ifeq ($(CONFIG_SUNRPC_XPRT_RDMA),n)
$(info RDMA transport: disabled)
CONFIG_SUNRPC_XPRT_RDMA_DUMMY := m
else
CONFIG_SUNRPC_XPRT_RDMA := m
CONFIG_SUNRPC_XPRT_RDMA_DUMMY := n
$(info RDMA transport: enabled)
endif
...
ifneq ($(compile_h),)
KERNEL_GCC_MAJOR := $(shell grep LINUX_COMPILER $(compile_h) | sed -r -e 's/.*gcc (version|[(]GCC[)]) ([0-9\.\-]*) .*/\2/g' | cut -d'.' -f1)
KERNEL_GCC_MINOR := $(shell grep LINUX_COMPILER $(compile_h) | sed -r -e 's/.*gcc (version|[(]GCC[)]) ([0-9\.\-]*) .*/\2/g' | cut -d'.' -f2)
KERNEL_GCC_PATCH := $(shell grep LINUX_COMPILER $(compile_h) | sed -r -e 's/.*gcc (version|[(]GCC[)]) ([0-9\.\-]*) .*/\2/g' | cut -d'.' -f3)
...
# Some custom kernels may be disabling NFS. This allows the module to compile regardless
...
NFS_CONFIGS = \
	...
	CONFIG_NFSD_V4_2_INTER_SSC=m \
	CONFIG_NFS_V4_2_SSC_HELPER=m \
	CONFIG_NFSD_V3=m \
	CONFIG_NFS_V4_1_IMPLEMENTATION_ID_DOMAIN=\"vastdata.com\" \

ifneq ($(OFA_DIR),)
default:
# compile with ofed driver
	./compat/collect-config -C $(K_BUILD) CC='$(CC)' O=$(K_OBJ) VASTNFS_OVERRIDES="${NFS_CONFIGS}" \
	...
	rm -f Module.symvers
	PATH=$$(pwd)/bundle/bin-wrap:$$PATH make -j8 -C $(K_BUILD) CC='$(CC)' O=$(K_OBJ) M=$(shell pwd) $(WITH_MAKE_PARAMS) \
		$$(cat compat.env) \
		$(NFS_CONFIGS) \
		CONFIG_NFS_MOUNT_TRACES=$(CONFIG_NFS_MOUNT_TRACES) \
		CONFIG_NFS_BUFFERED_INTERNAL_WRITEBACK_ENABLED=$(CONFIG_NFS_BUFFERED_INTERNAL_WRITEBACK_ENABLED) \
		CONFIG_NFSD_NEW_FAULT_INJECTION=$(CONFIG_NFSD_NEW_FAULT_INJECTION) \
		CONFIG_SUNRPC_XPRT_RDMA_DUMMY=$(CONFIG_SUNRPC_XPRT_RDMA_DUMMY) \
		$(CONFIG_NVSTUB) \
		CONFIG_DTRACE= \
		CONFIG_CTF= \
		LINUXINCLUDE='$(EXTRA_CFLAGS) -include compat.h -include $(autoconf_h) $(KCONFIG_H) -include $(OFA)/include/linux/compat-2.6.h -include compat-after.h  -I$(PWD) -I$(PWD)/compat/include -I$(OFA)/include -I$(OFA)/include/uapi -I$(PWD)/bundle/include -I$(PWD)/bundle/include/uapi $(BACKPORT_INCLUDES) $$(if $$(CONFIG_XEN),-D__XEN_INTERFACE_VERSION__=$$(CONFIG_XEN_INTERFACE_VERSION)) $$(if $$(CONFIG_XEN),-I$$(srctree)/arch/x86/include/mach-xen) -I$$(srctree)/arch/$$(SRCARCH)/include -Iarch/$$(SRCARCH)/include/generated -Iinclude -I$$(srctree)/arch/$$(SRCARCH)/include/uapi -Iarch/$$(SRCARCH)/include/generated/uapi -I$$(srctree)/include -I$$(srctree)/include/uapi -Iinclude/generated/uapi $$(if $$(KBUILD_SRC),-Iinclude2 -I$$(srctree)/include) -I$$(srctree)/arch/$$(SRCARCH)/include -Iarch/$$(SRCARCH)/include/generated' \
		modules
...
# compile without ofed driver
	./compat/collect-config -C $(K_BUILD) CC='$(CC)' O=$(K_OBJ) VASTNFS_OVERRIDES="${NFS_CONFIGS}"
	rm -f Module.symvers
	PATH=$$(pwd)/bundle/bin-wrap:$$PATH make -j8 -C $(K_BUILD) CC='$(CC)' O=$(K_OBJ) M=$(shell pwd) \
		$$(cat compat.env) \
		$(NFS_CONFIGS) \
		CONFIG_NFS_MOUNT_TRACES=$(CONFIG_NFS_MOUNT_TRACES) \
		CONFIG_NFS_BUFFERED_INTERNAL_WRITEBACK_ENABLED=$(CONFIG_NFS_BUFFERED_INTERNAL_WRITEBACK_ENABLED) \
...
rpmcheck:
	@which rpmbuild &> /dev/null; \
		if [ $$? -ne 0 ]; then \
		echo "*** rpmbuild is not installed (is this an rpm-based linux distribution?)"; \
		(exit 1); exit 1; \
		fi
