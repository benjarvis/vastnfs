 ...
 * Tracing XDR
 */
struct trace_xdr_buffer {
	u8 content[PAGE_SIZE];
};
extern struct trace_xdr_buffer *get_per_cpu_trace_xdr_buffer(unsigned int *cpu);

#define xdr_trace_encode_wrap(_prog_, _name_, _suffix_name_) \
	static void _prog_##_xdr_trace_enc_##_suffix_name_(struct rpc_rqst *req, \
						    struct xdr_stream *xdr, \
						    const void *data) \
	{ \
		if (trace_##_prog_##_encode_##_name_##_request_enabled()) { \
			struct trace_xdr_buffer *buffer; \
			\
			unsigned int prev_len = xdr->buf ? xdr->buf->len : 0, cpu; \
			unsigned int cur_len, size, orig_diff; \
			\
			_prog_##_xdr_enc_##_suffix_name_(req, xdr, data); \
			cur_len = xdr->buf ? xdr->buf->len : 0; \
			\
			buffer = get_per_cpu_trace_xdr_buffer(&cpu); \
			size = rpc_xdr_copy(req->rq_task, xdr, \
					    cur_len, prev_len, &orig_diff, \
					    buffer); \
			trace_##_prog_##_encode_##_name_##_request( \
				xdr, &buffer->content[0], size, orig_diff); \
			put_cpu(); \
		} else { \
			_prog_##_xdr_enc_##_suffix_name_(req, xdr, data); \
		} \
...
#define xdr_trace_decode_wrap(_prog_, _name_, _suffix_name_) \
	static int _prog_##_xdr_trace_dec_##_suffix_name_(struct rpc_rqst *req, \
						   struct xdr_stream *xdr, \
						   void *res) \
	{ \
		int status; \
		\
		if (trace_##_prog_##_decode_##_name_##_response_enabled()) { \
			struct trace_xdr_buffer *buffer; \
			\
			unsigned int prev_len = xdr_stream_pos(xdr), cur_len, cpu; \
			unsigned int size, orig_diff; \
			\
			status = _prog_##_xdr_dec_##_suffix_name_(req, xdr, res); \
			cur_len = xdr_stream_pos(xdr); \
			\
			buffer = get_per_cpu_trace_xdr_buffer(&cpu); \
			size = rpc_xdr_copy(req->rq_task, xdr, \
					    cur_len, prev_len, &orig_diff, \
					    buffer); \
			trace_##_prog_##_decode_##_name_##_response( \
				xdr, &buffer->content[0], size, orig_diff); \
			put_cpu(); \
		} else { \
			status = _prog_##_xdr_dec_##_suffix_name_(req, xdr, res); \
...
extern int rpc_xdr_copy(struct rpc_task *task,
			struct xdr_stream *xdr,
			uint32_t cur_len,
			uint32_t prev_pos,
			uint32_t *orig_diff,
			struct trace_xdr_buffer *buffer);
extern int alloc_per_cpu_trace_xdr_buffers(void);
extern void free_per_cpu_trace_xdr_buffers(void);
