...
struct nfs_inode {
	...
	struct timespec64	unlock_mtime;
	...
	struct list_head	async_write_list;
	atomic_t		async_write_count;
	spinlock_t		async_write_lock;
...
static inline struct nfs_server *NFS_SERVER(const struct inode *inode)
{
	...
	struct nfs_server *server = NFS_SB(inode->i_sb);

	if (nfsi->server_idx1 &&
	    nfsi->server_idx1 - 1 <= server->superblock.nr_alt_servers) {
		if (nfsi->server_idx1 == 1)
			return server;
		return server->superblock.alt_server[nfsi->server_idx1 - 2];
	}
...
extern struct inode *nfs_fhget_server(struct super_block *, struct nfs_fh *,
				      struct nfs_fattr *, struct nfs4_label *,
				      struct nfs_server *server, struct dentry *dentry);
static inline struct inode *nfs_fhget(struct super_block *sb, struct nfs_fh *fh,
				struct nfs_fattr *fattr, struct nfs4_label *label)
{
	return nfs_fhget_server(sb, fh, fattr, label, NULL, NULL);
}
...
extern int nfs_getattr(COMPAT_INIT_USER_NS_PARAM const struct path *,
		       struct kstat *, u32, unsigned int);
extern void nfs_access_add_cache(struct inode *, struct nfs_access_entry *);
extern void nfs_access_set_mask(struct nfs_access_entry *, u32);
extern int nfs_permission(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns) struct inode *, int);
...
extern int nfs_setattr(COMPAT_INIT_USER_NS_PARAM struct dentry *, struct iattr *);
extern void nfs_setattr_update_inode(struct inode *inode, struct iattr *attr, struct nfs_fattr *);
extern void nfs_setsecurity(struct inode *inode, struct nfs_fattr *fattr,
				struct nfs4_label *label);
extern struct nfs_open_context *get_nfs_open_context(struct nfs_open_context *ctx);
extern void put_nfs_open_context(struct nfs_open_context *ctx);
extern void put_nfs_open_context_sync(struct nfs_open_context *ctx);
extern struct nfs_open_context *nfs_find_open_context(struct inode *inode, const struct cred *cred, fmode_t mode);
extern struct nfs_open_context *alloc_nfs_open_context(struct nfs_server *server, struct dentry *dentry, fmode_t f_mode, struct file *filp);
...
extern int nfs_wb_folio_cancel(struct inode *inode, struct folio *folio);
#else
