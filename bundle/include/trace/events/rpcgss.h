...
DECLARE_EVENT_CLASS(rpcgss_gssapi_event,
	...
	TP_printk("task:%llx@%u maj_stat=%s",
...
TRACE_EVENT(rpcgss_unwrap_failed,
	...
	TP_printk("task:%llx@%u", __entry->task_id, __entry->client_id)
);
...
TRACE_EVENT(rpcgss_bad_seqno,
	...
	TP_printk("task:%llx@%u expected seqno %u, received seqno %u",
		__entry->task_id, __entry->client_id,
		__entry->expected, __entry->received)
);
...
TRACE_EVENT(rpcgss_seqno,
	...
	TP_printk("task:%llx@%u xid=0x%08x seqno=%u",
		__entry->task_id, __entry->client_id,
		__entry->xid, __entry->seqno)
);
...
TRACE_EVENT(rpcgss_need_reencode,
	...
	TP_printk("task:%llx@%u xid=0x%08x rq_seqno=%u seq_xmit=%u reencode %sneeded",
		__entry->task_id, __entry->client_id,
...
TRACE_EVENT(rpcgss_update_slack,
	...
	TP_printk("task:%llx@%u xid=0x%08x auth=%p rslack=%u ralign=%u verfsize=%u\n",
