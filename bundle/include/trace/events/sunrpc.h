...
DECLARE_EVENT_CLASS(rpc_xdr_buf_class,
	...
	TP_fast_assign(
		...
		__entry->client_id = task->tk_client ?
	        ...
	        __entry->xprt_id = task->tk_xprt ? task->tk_xprt->id : -1;
	...
	TP_printk("task:%llx@%u xprt=%d head=[%p,%zu] page=%u tail=[%p,%zu] len=%u",
...
TRACE_EVENT(rpc_clnt_new,
	TP_PROTO(
		...
		const struct rpc_xprt_switch *xps,
		const char *program,
		const char *server
	),

	TP_ARGS(clnt, xprt, xps, program, server),
	...
	TP_fast_assign(
		...
		__entry->xprt_id = xprt->id;
		__entry->xps_id = xps->xps_id;
		__assign_str(program, program);
		__assign_str(server, server);
	),

	TP_printk("client=%u xprt=%u xps=%u peer=[%s]:%s program=%s server=%s",
		__entry->client_id,
		__entry->xprt_id, __entry->xps_id,
		__get_str(addr), __get_str(port),
		__get_str(program), __get_str(server))
);

#define show_rpc_new_client_err(type)				\
	__print_symbolic(type,					\
		{ SOCK_STREAM,		"STREAM" },		\
		{ SOCK_DGRAM,		"DGRAM" },		\
		{ SOCK_RAW,		"RAW" },		\
...
TRACE_EVENT(rpc_clnt_register_err,
	TP_PROTO(
		const struct rpc_clnt *clnt,
		int error
	),
...
DECLARE_EVENT_CLASS(rpc_task_status,
	...
	TP_printk("task:%llx@%u xprt=%d status=%d",
		__entry->task_id, __entry->client_id, __entry->xprt_id,
		__entry->status)
);
...
TRACE_EVENT(rpc_request,
	...
	TP_printk("task:%llx@%u %sv%d %s (%ssync)",
...
TRACE_EVENT(rpc_route_start,
	TP_PROTO(const struct rpc_task *task, struct rpc_clnt *clnt, struct rpc_xprt_iter_conf *conf),

	TP_ARGS(task, clnt, conf),
	...
	TP_STRUCT__entry(
		...
		__field(int, version)
		__field(bool, async)
		__field(unsigned int, unc)
		__field(unsigned int, gpu)
		__field(unsigned int, gpu_idx)
		__field(unsigned int, best_priority)
		__field(unsigned int, rport)
		__field(unsigned int, rport_hash)
		__field(unsigned int, rport_nports)
		__field(unsigned int, rport_idx)
		__field(unsigned int, ign)
		__field(unsigned int, split)
		__field(unsigned int, marked)
	...
	TP_fast_assign(
		...
		__entry->async = RPC_IS_ASYNC(task);
		__entry->unc = !!(conf->flags & RPC_XPRT_FLAGS_SKIP_UNCONNECTED);
		__entry->gpu = !!(conf->flags & RPC_XPRT_FLAGS_LOCAL_GPU),
		__entry->gpu_idx = (conf->flags & RPC_XPRT_FLAGS_LOCAL_GPU) ? conf->gpu_idx : 0,
		__entry->best_priority = (conf->flags & RPC_XPRT_FLAGS_LOCAL_GPU) ? conf->best_priority : 0,
		__entry->rport = !!(conf->flags & RPC_XPRT_FLAGS_REMOTE_PORT_IDX),
		__entry->rport_idx = (conf->flags & RPC_XPRT_FLAGS_REMOTE_PORT_IDX) ? conf->remote_port_idx : 0,
		__entry->rport_hash = (conf->flags & RPC_XPRT_FLAGS_REMOTE_PORT_IDX) ? task->tk_multipath_hash : 0,
		__entry->rport_nports = (conf->flags & RPC_XPRT_FLAGS_REMOTE_PORT_IDX) ? (clnt->cl_remoteports_usage ? clnt->cl_remoteports_usage->nr : 0) : 0,
		__entry->ign = !!(conf->flags & RPC_XPRT_FLAGS_IGNORE_CONGESTION),
		__entry->split = !!(conf->flags & RPC_XPRT_FLAGS_SPLIT),
		__entry->marked = !!(conf->flags & RPC_XPRT_FLAGS_MARKED),
		__assign_str(progname, clnt->cl_program->name)
		__assign_str(procname, rpc_proc_name(task))
	),

	TP_printk("task:%llx@%u %sv%d %s%s(%d,%d)%s(%d,%d%%%d)%s %s %s %s (%ssync)",
		__entry->task_id, __entry->client_id,
		__get_str(progname), __entry->version,
		__entry->unc ? ":unc" : ":",
		__entry->gpu ? ":gpu" : ":",
		__entry->gpu_idx,
		__entry->best_priority,
		__entry->rport ? ":rport" : ":",
		__entry->rport_idx,
		__entry->rport_hash,
		__entry->rport_nports,
		__entry->ign ? ":ign" : ":",
		__entry->split ? ":split" : ":",
...
TRACE_EVENT(rpc_route_verdict,
	TP_PROTO(const struct rpc_task *task, struct rpc_clnt *clnt, struct rpc_xprt_iter_conf *conf, const char *verdict),

	TP_ARGS(task, clnt, conf, verdict),
	...
	TP_fast_assign(
		...
		__entry->rport_nports = (conf->flags & RPC_XPRT_FLAGS_REMOTE_PORT_IDX) ? (clnt->cl_remoteports_usage ? clnt->cl_remoteports_usage->nr : 0) : 0,
		__entry->ign = !!(conf->flags & RPC_XPRT_FLAGS_IGNORE_CONGESTION),
		__assign_str(verdict, verdict)
		__assign_str(progname, clnt->cl_program->name)
		__assign_str(procname, rpc_proc_name(task))
	),

	TP_printk("task:%llx@%u %sv%d %s%s(%d,%d)%s(%d,%d%%%d)%s->%s %s (%ssync)",
		__entry->task_id, __entry->client_id,
		__get_str(progname), __entry->version,
		__entry->unc ? ":unc" : ":",
		__entry->gpu ? ":gpu" : ":",
...
TRACE_EVENT(rpc_route_request,
	...
	TP_STRUCT__entry(
		...
		__field(int, version)
		__field(bool, async)
		__string(dstaddr, task->tk_xprt->address_strings[RPC_DISPLAY_ADDR])
		__string(progname, task->tk_client->cl_program->name)
		__string(procname, rpc_proc_name(task))
	),
	...
	TP_fast_assign(
		...
		__entry->version = task->tk_client->cl_vers;
		__entry->async = RPC_IS_ASYNC(task);
		__assign_str(dstaddr, task->tk_xprt->address_strings[RPC_DISPLAY_ADDR]);
		__assign_str(progname, task->tk_client->cl_program->name)
		__assign_str(procname, rpc_proc_name(task))
	),

	TP_printk("task:%llx@%u %sv%d %s %s (%ssync)",
		__entry->task_id, __entry->client_id,
		__get_str(progname), __entry->version,
		__get_str(dstaddr),
		__get_str(procname), __entry->async ? "a": ""
...
DECLARE_EVENT_CLASS(rpc_task_running,
	...
	TP_printk("task:%llx@%d xprt=%u flags=%s runstate=%s status=%d action=%ps",
		__entry->task_id, __entry->client_id, __entry->xprt_id,
...
DECLARE_EVENT_CLASS(rpc_task_queued,
	...
	TP_printk("task:%llx@%d xprt=%d flags=%s runstate=%s status=%d timeout=%lu queue=%s",
		__entry->task_id, __entry->client_id, __entry->xprt_id,
		rpc_show_task_flags(__entry->flags),
		rpc_show_runstate(__entry->runstate),
		__entry->status,
...
DECLARE_EVENT_CLASS(rpc_reply_event,
	...
	TP_printk("task:%llx@%d server=%s xid=0x%08x %sv%d %s",
...
TRACE_EVENT(rpc_buf_alloc,
	...
	TP_printk("task:%llx@%u xprt=%d callsize=%zu recvsize=%zu status=%d",
		__entry->task_id, __entry->client_id, __entry->xprt_id,
...
TRACE_EVENT(rpc_call_rpcerror,
	...
	TP_printk("task:%llx@%u xprt=%d tk_status=%d rpc_status=%d",
		__entry->task_id, __entry->client_id, __entry->xprt_id,
...
TRACE_EVENT(rpc_stats_latency,
	...
	TP_printk("task:%llx@%d xprt=%d xid=0x%08x %sv%d %s backlog=%lu rtt=%lu execute=%lu",
		__entry->task_id, __entry->client_id, __entry->xprt_id, __entry->xid,
...
TRACE_EVENT(rpc_xdr_overflow,
	...
	TP_printk(
		"task:%llx@%u %sv%d %s requested=%zu p=%p end=%p xdr=[%p,%zu]/%u/[%p,%zu]/%u\n",
		__entry->task_id, __entry->client_id,
...
TRACE_EVENT(rpc_xdr_alignment,
	...
	TP_printk(
		"task:%llx@%u %sv%d %s offset=%zu copied=%u xdr=[%p,%zu]/%u/[%p,%zu]/%u\n",
		__entry->task_id, __entry->client_id,
		__get_str(progname), __entry->version, __get_str(procedure),
...
#define rpc_show_task_state_transition(st) \
	__print_symbolic(st, RPC_TASK_STATE_TRANSITION)
...
DECLARE_EVENT_CLASS(xs_socket_event_done,
		...
		TP_printk(
			"error=%d xprt=%d socket:[%llu] dstaddr=%s/%s "
			"state=%u (%s) sk_state=%u (%s)",
			__entry->error, __entry->xprt_id,
			__entry->ino, __get_str(dstaddr), __get_str(dstport),
			__entry->socket_state,
			rpc_show_socket_state(__entry->socket_state),
			__entry->sock_state,
...
TRACE_EVENT(rpc_tcp_socket_timer,
	TP_PROTO(
		struct rpc_xprt *xprt,
		struct sock *sock,
		struct socket *socket,
		...
		unsigned int icsk_pending
	),

	TP_ARGS(xprt, sock, socket, delta, timer_active, timeout, icsk_pending),
	...
	TP_STRUCT__entry(
		...
		__field(unsigned int, timer_active)
		__field(unsigned int, timeout)
		__field(unsigned int, icsk_pending)
	),

	TP_fast_assign(
		__entry->xprt_id = xprt->id;
		__entry->socket_state = socket->state;
		__entry->sock_state = sock->sk_state;
		__entry->timer_active = timer_active;
		__entry->timeout = timeout;
		__entry->delta = delta;
		__entry->icsk_pending = icsk_pending;
	),

	TP_printk(
		"xprt=%d state=%u (%s) sk_state=%u (%s) "
		"delta=%ld timer_active=%d timeout=%d icsk_pending=%d",
		__entry->xprt_id,
		__entry->socket_state, rpc_show_socket_state(__entry->socket_state),
		__entry->sock_state, rpc_show_sock_state(__entry->sock_state),
		__entry->delta, __entry->timer_active, __entry->timeout,
		__entry->icsk_pending
	)
);
...
TRACE_EVENT(rpc_socket_nospace,
	...
	TP_printk("task:%llx@%u total=%u remaining=%u",
		__entry->task_id, __entry->client_id,
		__entry->total, __entry->remaining
	)
);

DECLARE_EVENT_CLASS(xprt_lifetime_class,
	TP_PROTO(
		const struct rpc_xprt *xprt
	),
	...
	TP_printk("xprt=%u", __entry->xprt_id)
);

#define DEFINE_NFS_CLIENT_FREE_CLASS(name) \
	DEFINE_EVENT(xprt_lifetime_class, name, \
		     TP_PROTO(const struct rpc_xprt *clp), \
		     TP_ARGS(clp));

DEFINE_NFS_CLIENT_FREE_CLASS(xprt_alloc);
DEFINE_NFS_CLIENT_FREE_CLASS(xprt_free);
...
DECLARE_EVENT_CLASS(rpc_xprt_lifetime_class,
	...
	TP_printk("xprt=%u peer=[%s]:%s state=%s",
		__entry->xprt_id, __get_str(addr), __get_str(port),
...
DECLARE_EVENT_CLASS(xprt_switch_lifetime_class,
	TP_PROTO(
		const struct rpc_xprt_switch *xprt_switch
	),
	...
	TP_fast_assign(
		__entry->xps_id = xprt_switch->xps_id;
	),

	TP_printk("xps=%u", __entry->xps_id)
);

#define DEFINE_XPRT_SWITCH_ALLOCATION_CLASS(name) \
	DEFINE_EVENT(xprt_switch_lifetime_class, name, \
		     TP_PROTO(const struct rpc_xprt_switch *xprt_switch), \
		     TP_ARGS(xprt_switch));

DEFINE_XPRT_SWITCH_ALLOCATION_CLASS(xprt_switch_alloc);
DEFINE_XPRT_SWITCH_ALLOCATION_CLASS(xprt_switch_free);

DECLARE_EVENT_CLASS(xprt_switch_bind_lifetime_class,
	TP_PROTO(
		const struct rpc_xprt_switch *xprt_switch,
		const struct rpc_xprt *xprt
	),

	TP_ARGS(xprt_switch, xprt),
	...
	TP_printk("xps=%u xprt=%u", __entry->xps_id, __entry->xprt_id)
);

#define DEFINE_XPRT_SWITCH_BINDING_CLASS(name) \
	DEFINE_EVENT(xprt_switch_bind_lifetime_class, name, \
		     TP_PROTO(const struct rpc_xprt_switch *xprt_switch, \
			      const struct rpc_xprt *xprt), \
		     TP_ARGS(xprt_switch, xprt));

DEFINE_XPRT_SWITCH_BINDING_CLASS(xprt_switch_bind);
DEFINE_XPRT_SWITCH_BINDING_CLASS(xprt_switch_unbind);
...
DECLARE_EVENT_CLASS(rpc_xprt_event,
	...
	TP_printk("xprt=%d peer=[%s]:%s xid=0x%08x status=%d", __entry->xprt_id,
			__get_str(addr),
...
TRACE_EVENT(xprt_transmit,
	...
	TP_fast_assign(
		...
		__entry->xprt_id = rqst->rq_task->tk_xprt ?
                        rqst->rq_task->tk_xprt->id : -1;
	...
	TP_printk(
		"task:%llx@%u xprt=%d xid=0x%08x seqno=%u status=%d",
		__entry->task_id, __entry->client_id, __entry->xprt_id,
		__entry->xid,
...
TRACE_EVENT(xprt_retransmit,
	...
	TP_printk(
		"task:%llx@%u xprt=%d xid=0x%08x %sv%d %s ntrans=%d timeout=%lu",
		__entry->task_id, __entry->client_id, __entry->xprt_id, __entry->xid,
		__get_str(progname), __entry->version, __get_str(procname),
...
TRACE_EVENT(xprt_ping,
	...
	TP_printk("xprt=%d peer=[%s]:%s status=%d",
		__entry->xprt_id, __get_str(addr), __get_str(port), __entry->status)
);
...
DECLARE_EVENT_CLASS(xprt_writelock_event,
	...
	TP_printk("task:%llx@%u xprt=%d snd_task:%llx",
			__entry->task_id, __entry->client_id, __entry->xprt_id,
			__entry->snd_task_id)
);
...
DECLARE_EVENT_CLASS(xprt_cong_event,
	...
	TP_printk("task:%llx@%u xprt=%d snd_task:%llx cong=%lu cwnd=%lu%s",
			__entry->task_id, __entry->client_id, __entry->xprt_id,
...
TRACE_EVENT(xprt_reserve,
	...
	TP_printk("task:%llx@%u xprt=%d xid=0x%08x",
		__entry->task_id, __entry->client_id,
		__entry->xprt_id, __entry->xid
	)
);
...
TRACE_EVENT(xs_stream_read_data,
	...
	TP_fast_assign(
		__entry->err = err;
		__entry->total = total;
		__entry->xprt_id = xprt ? xprt->id : -1;
	...
	TP_printk("xprt=%d peer=[%s]:%s err=%zd total=%zu", __entry->xprt_id,
		  __get_str(addr), __get_str(port), __entry->err, __entry->total)
);
...
TRACE_EVENT(xs_stream_read_request,
	...
	TP_printk("xprt=%d peer=[%s]:%s xid=0x%08x copied=%lu reclen=%u offset=%u",
		  __entry->xprt_id, __get_str(addr), __get_str(port), __entry->xid,
...
TRACE_EVENT(rpcb_getport,
	...
	TP_printk("task:%llx@%u server=%s program=%u version=%u protocol=%d bind_version=%u",
		__entry->task_id, __entry->client_id, __get_str(servername),
...
TRACE_EVENT(rpcb_setport,
	...
	TP_printk("task:%llx@%u status=%d port=%u",
		__entry->task_id, __entry->client_id,
...
TRACE_EVENT(rpc_task_state_transition,
	TP_PROTO(const struct rpc_task *task, int st),
	...
	TP_fast_assign(
		__entry->task_id = task->tk_pid;
		__entry->client_id = task->tk_client ? task->tk_client->cl_clid : -1;
		__entry->xprt_id = task->tk_xprt ? task->tk_xprt->id : -1;
		__entry->status = task->tk_status;
		__entry->st = st;
	),

	TP_printk("task:%llx@%u xprt=%d status=%d st=%s",
		__entry->task_id, __entry->client_id, __entry->xprt_id,
		__entry->status, rpc_show_task_state_transition(__entry->st))
)

TRACE_EVENT(walltime_correlation,
	TP_PROTO(int dummy),
	TP_ARGS(dummy),
	...
	TP_fast_assign(
		...
		__entry->tv_sec = now.tv_sec;
		__entry->tv_nsec = now.tv_nsec;
	),

	TP_printk("tv_sec=%lld tv_nsec=%d", __entry->tv_sec, __entry->tv_nsec)
);
