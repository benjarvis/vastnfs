...
DECLARE_EVENT_CLASS(xprtrdma_rxprt,
	...
	TP_fast_assign(
		...
		__entry->xprt_id = r_xprt->rx_xprt.id;
	),

	TP_printk("peer=[%s]:%s xprt=%d",
		__get_str(addr), __get_str(port), __entry->xprt_id
	)
);
...
DECLARE_EVENT_CLASS(xprtrdma_connect_class,
	...
	TP_fast_assign(
		...
		__entry->rc = rc;
		__entry->ep = ep ? 1 : 0;
		__entry->connect_status = ep ? ep->re_connect_status : 0;
		__entry->xprt_id = r_xprt->rx_xprt.id;
		__assign_str(addr, rpcrdma_addrstr(r_xprt));
		__assign_str(port, rpcrdma_portstr(r_xprt));
	),

	TP_printk("peer=[%s]:%s xprt=%d rc=%d ep=%d connection_status=%d",
		__get_str(addr), __get_str(port), __entry->xprt_id,
		__entry->rc, __entry->ep, __entry->connect_status
	)
);
...
DECLARE_EVENT_CLASS(xprtrdma_rdch_event,
	...
	TP_printk("task:%llx@%u pos=%u %u@0x%016llx:0x%08x (%s)",
...
DECLARE_EVENT_CLASS(xprtrdma_wrch_event,
	...
	TP_printk("task:%llx@%u %u@0x%016llx:0x%08x (%s)",
		__entry->task_id, __entry->client_id,
...
DECLARE_EVENT_CLASS(xprtrdma_mr_class,
	...
	TP_fast_assign(
		...
		__entry->task_id = task->tk_pid;
		__entry->client_id = task->tk_client->cl_clid;
		__entry->mr_id  = RDMA_RESTRACK_ENTRY_ID(&mr->mr_ibmr->res);
		__entry->nents  = mr->mr_nents;
		__entry->handle = mr->mr_handle;
		__entry->length = mr->mr_length;
		__entry->offset = mr->mr_offset;
	...
	TP_printk("task:%llx@%u mr.id=%u nents=%d %u@0x%016llx:0x%08x (%s)",
		__entry->task_id, __entry->client_id,
...
DECLARE_EVENT_CLASS(xprtrdma_callback_class,
	...
	TP_printk("peer=[%s]:%s xprt=%d xid=0x%08x",
		__get_str(addr), __get_str(port), __entry->xprt_id, __entry->xid
	)
);
...
TRACE_EVENT(xprtrdma_op_connect,
	...
	TP_printk("peer=[%s]:%s xprt=%d delay=%lu",
		__get_str(addr), __get_str(port), __entry->xprt_id,
		__entry->delay
	)
);
...
TRACE_EVENT(xprtrdma_op_set_cto,
	...
	TP_printk("peer=[%s]:%s xprt=%d connect=%lu reconnect=%lu",
		__get_str(addr), __get_str(port), __entry->xprt_id,
...
TRACE_EVENT(xprtrdma_createmrs,
	...
	TP_printk("peer=[%s]:%s xprt=%d created %u MRs",
		__get_str(addr), __get_str(port), __entry->xprt_id,
		__entry->count
	)
);
...
TRACE_EVENT(xprtrdma_nomrs_err,
	...
	TP_printk("peer=[%s]:%s xprt=%d task:%llx@%u",
		__get_str(addr), __get_str(port), __entry->xprt_id,
		__entry->task_id, __entry->client_id
	)
);
...
DECLARE_EVENT_CLASS(xprtrdma_cm_event_class,
	TP_PROTO(
	...
	TP_printk("xprt=%d status=%d event=%s (%ld)",
		__entry->xprt_id, __entry->status,
		rdma_show_cm_event(__entry->event), __entry->event)
);

#define DEFINE_XPRTRDMA_CM_EVENT(name) \
		DEFINE_EVENT(xprtrdma_cm_event_class, name, \
				TP_PROTO( \
					const struct rpc_xprt *xprt, \
					const struct rdma_cm_event *event \
				), \
...
DEFINE_XPRTRDMA_CM_EVENT(xprtrdma_cm_event);
DEFINE_XPRTRDMA_CM_EVENT(xprtrdma_cm_event_err);
...
#define rpc_show_xprtrdma_state_transition(st) \
	__print_symbolic(st, RPC_XPRTRDMA_STATE_TRANSITION)

RPC_XPRTRDMA_STATE_TRANSITION
...
TRACE_EVENT(xprtrdma_state_transition,
	TP_PROTO(struct rpcrdma_xprt *r_xprt,
		 enum rpc_xprtrdma_state_transition st),

	TP_ARGS(r_xprt, st),

	TP_STRUCT__entry(
		__field(unsigned int, xprt_id)
		__field(enum rpc_xprtrdma_state_transition, st)
	),
	...
	TP_printk("xprt=%d st=%s",
		__entry->xprt_id,
		rpc_show_xprtrdma_state_transition(__entry->st))
)
...
TRACE_EVENT(xprtrdma_marshal,
	...
	TP_printk("task:%llx@%u xid=0x%08x: hdr=%u xdr=%u/%u/%u %s/%s",
...
TRACE_EVENT(xprtrdma_marshal_failed,
	...
	TP_printk("task:%llx@%u xid=0x%08x: ret=%d",
		__entry->task_id, __entry->client_id, __entry->xid,
		__entry->ret
	)
);
...
TRACE_EVENT(xprtrdma_post_send,
	...
	TP_printk("task:%llx@%u cq.id=%u cid=%d (%d SGE%s) %s",
		__entry->task_id, __entry->client_id,
		__entry->cq_id, __entry->completion_id,
...
TRACE_EVENT(xprtrdma_post_send_err,
	...
	TP_fast_assign(
		...
		__entry->cq_id = ep ? RDMA_RESTRACK_ENTRY_ID(&ep->re_attr.recv_cq->res) : 0;
		__entry->xprt_id = r_xprt->rx_xprt.id;
		__entry->task_id = rqst->rq_task->tk_pid;
		__entry->client_id = rqst->rq_task->tk_client ?
				     rqst->rq_task->tk_client->cl_clid : -1;
	...
	TP_printk("task:%llx@%u xprt=%d cq.id=%u rc=%d",
		__entry->task_id, __entry->xprt_id, __entry->client_id,
...
TRACE_EVENT(xprtrdma_post_recvs,
	...
	TP_fast_assign(
		...
		__entry->cq_id = RDMA_RESTRACK_ENTRY_ID(&ep->re_attr.recv_cq->res);
		__entry->xprt_id = r_xprt->rx_xprt.id;
		__entry->count = count;
...
TRACE_EVENT(xprtrdma_post_recvs_err,
	...
	TP_printk("peer=[%s]:%s xprt=%d cq.id=%d rc=%d",
		__get_str(addr), __get_str(port), __entry->xprt_id,
		__entry->cq_id, __entry->status
	)
);
...
TRACE_EVENT(xprtrdma_reply,
	...
	TP_printk("task:%llx@%u xid=0x%08x credits=%u",
		__entry->task_id, __entry->client_id, __entry->xid,
		__entry->credits
	)
);
...
TRACE_EVENT(xprtrdma_err_vers,
	...
	TP_printk("task:%llx@%u xid=0x%08x versions=[%u, %u]",
		__entry->task_id, __entry->client_id, __entry->xid,
...
TRACE_EVENT(xprtrdma_err_chunk,
	...
	TP_printk("task:%llx@%u xid=0x%08x",
		__entry->task_id, __entry->client_id, __entry->xid
	)
);
...
TRACE_EVENT(xprtrdma_err_unrecognized,
	...
	TP_printk("task:%llx@%u xid=0x%08x procedure=%u",
		__entry->task_id, __entry->client_id, __entry->xid,
		__entry->procedure
	)
);
...
TRACE_EVENT(xprtrdma_fixup,
	...
	TP_printk("task:%llx@%u fixup=%lu xdr=%zu/%u/%zu",
...
TRACE_EVENT(xprtrdma_cb_setup,
	...
	TP_printk("peer=[%s]:%s xprt=%d %u reqs",
		__get_str(addr), __get_str(port),
		__entry->xprt_id, __entry->reqs
	)
);
