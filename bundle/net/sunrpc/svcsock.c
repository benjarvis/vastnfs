				...
				size_t seek)
{
	...
	compat_iov_iter_bvec(&msg.msg_iter, READ, bvec, i, buflen);
	if (seek) {
...
static void svc_tcp_kill_temp_xprt(struct svc_xprt *xprt)
{
	...
	compat_sock_no_linger(svsk->sk_sock);
}
...
static struct svc_xprt *svc_tcp_accept(struct svc_xprt *xprt)
{
	...
	err = compat_kernel_getpeername(newsock, sin);
	if (err < 0) {
	...
	err = compat_kernel_getsockname(newsock, sin);
	slen = err;
	if (unlikely(err < 0))
				   ...
				   struct svc_rqst *rqstp)
{
	...
	if (svsk->sk_tcplen < sizeof(rpc_fraghdr)) {
		...
		iov.iov_len  = want;
		compat_iov_iter_kvec(&msg.msg_iter, READ, &iov, 1, want);
		len = sock_recvmsg(svsk->sk_sock, &msg, MSG_DONTWAIT);
		if (len < 0)
			return len;
		svsk->sk_tcplen += len;
...
static int svc_tcp_send_kvec(struct socket *sock, const struct kvec *vec,
			      int flags)
{
#if defined(COMPAT_DETECT_MSG_SPLICE_PAGES)
...
static int svc_tcp_sendto(struct svc_rqst *rqstp)
{
	...
	if (svc_xprt_is_dead(xprt))
		goto out_notconn;
	compat_tcp_sock_set_cork(svsk->sk_sk, true);
	...
	if (atomic_dec_and_test(&svsk->sk_sendqlen))
		compat_tcp_sock_set_cork(svsk->sk_sk, false);
	mutex_unlock(&xprt->xpt_mutex);
	return sent;
		...
		const size_t len, const struct cred *cred)
{
	...
	salen = compat_kernel_getsockname(svsk->sk_sock, sin);
	if (salen >= 0)
					  ...
					  int flags)
{
	...
	error = compat_kernel_getsockname(sock, newsin);
	if (error < 0)
		goto bummer;
	newlen = error;
