...
#ifndef COMPAT_DETECT_FREE_INODE
static void rpc_i_callback(struct rcu_head *head)
{
	struct inode *inode = container_of(head, struct inode, i_rcu);
	rpc_free_inode(inode);
}
...
#if !defined(COMPAT_DETECT_FSNOTIFY_RMDIR) && !defined(COMPAT_DETECT_FSNOTIFY_NAMEREMOVE)
	(void)ret;
	d_delete(dentry);
#else
	d_drop(dentry);
...
#if !defined(COMPAT_DETECT_FSNOTIFY_UNLINK) && !defined(COMPAT_DETECT_FSNOTIFY_NAMEREMOVE)
	(void)ret;
	d_delete(dentry);
#else
	d_drop(dentry);
...
COMPAT_FS_CONTEXT_DETECTOR_IMPL(rpc_pipe_fs_type, rpc_init_fs_context, NULL);
...
int register_rpc_pipefs(void)
{
	...
	err = COMPAT_REGISTER_FILESYSTEM(rpc_pipe_fs_type);
	if (err)
		goto err_register;
	return 0;
...
{
	rpc_clients_notifier_unregister();
	COMPAT_UNREGISTER_FILESYSTEM(rpc_pipe_fs_type);
	kmem_cache_destroy(rpc_inode_cachep);
}
