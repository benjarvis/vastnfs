...
#define RPC_CREDCACHE_DEFAULT_HASHBITS	(9)
...
module_param_named(auth_number_cred_unused, number_cred_unused, ulong, 0444);
MODULE_PARM_DESC(auth_number_cred_unused, "RPC credential cache hashtable unused count [informational]");

static unsigned long auth_max_cred_cachesize = USHRT_MAX;
		...
		int flags, gfp_t gfp)
{
	...
	nr = auth->au_ops->hash_cred(auth, acred, cache->hashbits);
...
rpcauth_lookupcred(struct rpc_auth *auth, int flags)
{
	...
	acred.cred = cred;
	acred.pid = get_task_pid(current, PIDTYPE_SID);
