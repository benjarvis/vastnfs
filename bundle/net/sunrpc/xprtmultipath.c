		...
		struct rpc_xprt *xprt, struct rpc_xids **old_xids)
{
	...
	struct list_head *remoteport_list;
	...
	*old_xids = xchg(&xprt->xid_space, xids);
	...
	remoteport_list = &xps->xps_remoteport_xprt_list[xprt->remote_port_idx];
	list_add_tail_rcu(&xprt->remote_port_xprt_switch, remoteport_list);
		...
		struct rpc_xprt *xprt)
{
	...
	trace_xprt_switch_bind(xps, xprt);
		...
		struct rpc_xprt *xprt)
{
	...
	spin_unlock(&xps->xps_lock);
	trace_xprt_switch_unbind(xps, xprt);
	xprt_put(xprt);
}
...
static int xprt_switch_alloc_id(struct rpc_xprt_switch *xps, gfp_t gfp_flags)
{
	...
	id = ida_simple_get(&rpc_xprtswitch_ids, 2000, 0, gfp_flags);
	if (id < 0)
		return id;
...
bool xprt_is_active(const struct rpc_xprt *xprt, struct rpc_xprt_iter_conf conf)
{
	...
	if (conf.flags & RPC_XPRT_FLAGS_SPLIT) {
		/*
		 * Match based on mark. For the non-RPC_XPRT_FLAGS_REMOTE_PORT_IDX
		 * case we can assume there will non-zero of either kinds in the
		 * global iteration.
		 */
		bool reg_xprt = xprt->role == RPC_XPRT_ROLE_REG;
		bool marked = conf.flags & RPC_XPRT_FLAGS_MARKED;
		if (reg_xprt == marked)
			return false;
	}
