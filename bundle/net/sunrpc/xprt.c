...
#ifdef CONFIG_NVFS
static bool (*page_gpu_idx)(struct page *, unsigned int *idx);
#endif

static int	xprt_init_xid(struct rpc_xprt *xprt);
...
#ifdef CONFIG_NVFS
	if (!page_gpu_idx && transport->page_gpu_idx)
		page_gpu_idx = transport->page_gpu_idx;
#endif
...
int xprt_adjust_timeout(struct rpc_rqst *req)
{
	...
	trace_rpc_task_state_transition(req->rq_task,
					RPC_TASK_ST_TIMEOUT_CHECK);
	...
	if (time_before(jiffies, req->rq_majortimeo)) {
		...
		trace_rpc_task_state_transition(
			req->rq_task, RPC_TASK_ST_TIMEOUT_RETRY);
	} else {
		...
		spin_unlock(&xprt->transport_lock);
		trace_rpc_task_state_transition(
			req->rq_task, RPC_TASK_ST_TIMEOUT_MAJOR);
		status = -ETIMEDOUT;
	}
	xprt_reset_minortimeo(req);

	if (xprt->ops->trace_transport_state)
		xprt->ops->trace_transport_state(xprt);
...
xprt_request_enqueue_receive(struct rpc_task *task)
{
	...
	trace_rpc_task_state_transition(task, RPC_TASK_ST_QUEUE_RECV);
...
xprt_request_enqueue_transmit(struct rpc_task *task)
{
	...
	trace_rpc_task_state_transition(task, RPC_TASK_ST_READY_QUEUE_XMIT);
	...
	if (xprt_request_need_enqueue_transmit(task, req)) {
		...
		if (req->rq_cong) {
			...
			list_for_each_entry(pos, &xprt->xmit_queue, rq_xmit) {
				...
				trace_rpc_task_state_transition(task,
					RPC_TASK_ST_QUEUE_XMIT_CONG);
				goto out;
			}
			...
			list_for_each_entry(pos, &xprt->xmit_queue, rq_xmit) {
				...
				trace_rpc_task_state_transition(task,
					RPC_TASK_ST_QUEUE_XMIT_NOSEQ);
				goto out;
			}
		} else {
			trace_rpc_task_state_transition(task,
				RPC_TASK_ST_QUEUE_XMIT);
		}
...
#ifdef CONFIG_NVFS
/**
 * xprt_page_gpu_idx - Determine a gpu idx for this page.
 * @page: pointer to the page
 * @gpu_idx: pointer to output
 *
 * Returns true of the gpu idx was written to *gpu_dix.
 */
bool xprt_page_gpu_idx(struct page *page, unsigned int *gpu_idx)
{
	bool (*fn)(struct page *, unsigned int *) = READ_ONCE(page_gpu_idx);
	if (fn)
		return fn(page, gpu_idx);
...
{
	switch (type) {
		case XPRT_TRANSPORT_UDP: return "udp";
		case XPRT_TRANSPORT_TCP: return "tcp";
		case XPRT_TRANSPORT_BC_TCP: return "bc-tcp";
		case XPRT_TRANSPORT_RDMA: return "rdma";
		case XPRT_TRANSPORT_BC_RDMA: return "bc-rdma";
		case XPRT_TRANSPORT_LOCAL: return "local";
		default: return "?";
	}
}
EXPORT_SYMBOL(xprt_type_to_string);
		...
		unsigned int max_alloc)
{
	...
	xprt_alloc_id(xprt);
	trace_xprt_alloc(xprt);

	ret = xprt_init_xid(xprt);
	if (ret) {
		xprt_free_id(xprt);
		goto out_free;
	}
...
void xprt_free(struct rpc_xprt *xprt)
{
	...
	trace_xprt_free(xprt);
	xprt_free_id(xprt);
	rpc_sysfs_xprt_destroy(xprt);
	xid_space_put(xprt->xid_space);
	kfree_rcu(xprt, rcu);
}
...
{
	return (__force __be32)xid_space_new_xid(xprt->xid_space);
}
...
static void _xprt_portusage_undivert(struct xprt_portusage *usage, int *diversion_idx)
{
	if (*diversion_idx >= 0  &&  *diversion_idx < usage->nr) {
		struct xprt_port *port = &usage->ports[*diversion_idx];
		port->xprt_diversions -= 1;
		*diversion_idx = -1;
	}
}

void xprt_portusage_connect(struct xprt_portusage *usage,
			    struct sockaddr *sockaddr,
			    int *diversion_idx)
{
	int i, found = -1;

	spin_lock(&usage->lock);

	_xprt_portusage_undivert(usage, diversion_idx);

	for (i = 0; i < usage->nr; i++) {
		if (!rpc_cmp_addr_port(
			(struct sockaddr *)&usage->ports[i].addr, sockaddr))
			continue;

		usage->ports[i].xprt_connections += 1;
		found = 1;
		break;
	}

	spin_unlock(&usage->lock);
}

void xprt_portusage_disconnect(struct xprt_portusage *usage,
			       struct sockaddr *sockaddr)
{
	int i, found = -1;
...
void xprt_portusage_undivert(struct xprt_portusage *usage, int *diversion_idx)
{
	spin_lock(&usage->lock);
	_xprt_portusage_undivert(usage, diversion_idx);
	spin_unlock(&usage->lock);
...
int xprt_portusage_divert(struct xprt_portusage *usage,
			  struct sockaddr *in_sockaddr,
			  struct sockaddr_storage *out_sockaddr,
			  size_t *out_sockaddr_len)
{
	...
	for (i = 0; i < usage->nr; i++) {
		...
		if (rpc_cmp_addr_port((struct sockaddr *)&port->addr, in_sockaddr))
			continue;

		if (port->xprt_connections == 0)
			continue;

		total_usage = port->xprt_connections + port->xprt_diversions;
		if (total_usage < lowest) {
			minimum_idx = i;
			lowest = total_usage;
		}
	}
	...
	if (minimum_idx != -1) {
		struct xprt_port *port = &usage->ports[minimum_idx];
		*out_sockaddr = port->addr;
		*out_sockaddr_len = port->len;
		port->xprt_diversions += 1;
	}
...
{
	struct xprt_portusage *usage;

	usage = kzalloc(sizeof(*usage), GFP_KERNEL);
	if (!usage)
		return NULL;

	kref_init(&usage->kref);
	spin_lock_init(&usage->lock);
	return usage;
}
				...
				int addr_len)
{
	...
	spin_lock(&usage->lock);
	if (usage->nr >= XPRT_MAX_PORTS) {
		spin_unlock(&usage->lock);
		return -1;
	}

	for (j = 0; j < usage->nr; j++) {
		if (rpc_cmp_addr_port((struct sockaddr *)&usage->ports[j].addr, addr)) {
			spin_unlock(&usage->lock);
			return j;
		}
	}

	memcpy(&usage->ports[usage->nr].addr, addr, addr_len);
	usage->ports[usage->nr].len = addr_len;
	j = usage->nr;
	usage->nr++;
...
static const struct rpc_call_ops rpc_cb_nconnect_revive_call_ops;

static void rpc_cb_nconnect_revive_done(struct rpc_task *task, void *calldata)
{
	struct rpc_rqst *rqst = task->tk_rqstp;
	struct rpc_xprt *xprt = rqst->rq_xprt;

	dprintk("xprt=%p, revival task done, status=%d",
		xprt, task->tk_status);

	if (task->tk_status == 0) {
		xprt_clear_n_recovery(xprt);
		xprt_clear_n_diversion(xprt);
	} else {
		if (RPC_SIGNALLED(task) || kref_read(&xprt->kref) == 0)
			return;
		...
		task = rpc_call_null_helper(
			task->tk_client, xprt, NULL,
			RPC_TASK_ASYNC | RPC_TASK_NULLCREDS,
			&rpc_cb_nconnect_revive_call_ops, NULL);

		if (!IS_ERR(task)) {
			dprintk("xprt=%p, recreated revival task\n", xprt);
			rpc_put_task(task);
			return;
		}
	}
...
static void rpc_cb_nconnect_revive_release(void *calldata)
{
}

static const struct rpc_call_ops rpc_cb_nconnect_revive_call_ops = {
	.rpc_call_done = rpc_cb_nconnect_revive_done,
	.rpc_release = rpc_cb_nconnect_revive_release,
};

void
xprt_diversion_recovery_kickoff(struct rpc_clnt *client, struct rpc_xprt *xprt)
{
	struct rpc_task *task;

	dprintk("xprt=%p, creating revival task\n", xprt);
	...
	if (!IS_ERR(task)) {
		dprintk("xprt=%p, created revival task\n", xprt);
		rpc_put_task(task);
		return;
	}

	dprintk("xprt=%p, ERROR creating revival task\n", xprt);
}

EXPORT_SYMBOL(xprt_portusage_disconnect);
EXPORT_SYMBOL(xprt_portusage_connect);
EXPORT_SYMBOL(xprt_portusage_undivert);
EXPORT_SYMBOL(xprt_portusage_divert);
EXPORT_SYMBOL(xprt_diversion_recovery_kickoff);

static void kprt_portusage_destory(struct kref *kref)
{
	kfree(container_of(kref, struct rpc_xprt, kref));
}
...
{
	if (usage)
		kref_put(&usage->kref, kprt_portusage_destory);
}
...
struct rpc_xprt *xprt_create_transport(struct xprt_create *args)
{
	...
	if (args->localport_usage) {
		xprt_portusage_get(args->localport_usage);
		xprt->localport_usage = args->localport_usage;
	}
	xprt->remote_port_idx = args->remote_port_idx;
	xprt->role = args->role;
...
static void xprt_destroy_cb(struct work_struct *work)
{
	...
	localport_usage = xprt->localport_usage;
	/*
	...
	/*
	 * Discard port usage if needed.
	 */
	xprt_portusage_put(localport_usage);
}
...
struct rpc_xids *xid_space_alloc(void)
{
	...
	xids = kzalloc(sizeof(*xids), GFP_KERNEL);
	if (xids == NULL)
		return ERR_PTR(-ENOMEM);

	kref_init(&xids->kref);
	atomic_set(&xids->next, prandom_u32());
	return xids;
}
...
{
	return (__force u32)atomic_inc_return(&xids->next);
}

static void xid_space_destroy(struct kref *kref)
{
	kfree(container_of(kref, struct rpc_xids, kref));
}
...
void xid_space_put(struct rpc_xids *xids)
{
	...
	kref_put(&xids->kref, xid_space_destroy);
}
