...
svc_rdma_get_rw_ctxt(struct svcxprt_rdma *rdma, unsigned int sges)
{
	...
	if (compat_sg_alloc_table_chained(&ctxt->rw_sg_table, sges,
...
{
	compat_sg_free_table_chained(&ctxt->rw_sg_table, SG_CHUNK_SIZE);
...
{
	cid->ci_queue_id = RDMA_RESTRACK_ENTRY_ID(&rdma->sc_sq_cq->res);
...
static int svc_rdma_post_chunk_ctxt(struct svc_rdma_chunk_ctxt *cc)
{
	...
	COMPAT_IB_WR_CONST struct ib_send_wr *bad_wr;
	...
	do {
		if (atomic_sub_return(cc->cc_sqecount,
				      &rdma->sc_sq_avail) > 0) {
			ret = ib_post_send(rdma->sc_qp, first_wr,
					   &bad_wr);
			if (ret)
				break;
			return 0;
