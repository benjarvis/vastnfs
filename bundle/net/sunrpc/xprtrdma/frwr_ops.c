			  ...
			  struct rpcrdma_mr *mr)
{
	...
	cid->ci_queue_id = RDMA_RESTRACK_ENTRY_ID(&ep->re_attr.send_cq->res);
	cid->ci_completion_id = RDMA_RESTRACK_ENTRY_ID(&mr->mr_ibmr->res);
}
...
int frwr_query_device(struct rpcrdma_ep *ep, const struct ib_device *device)
{
	...
	max_sge = min_t(unsigned int, compat_attr_get_max_send_sge(attrs),
