...
static const struct rpc_pipe_ops gss_upcall_ops_v2;
...
__gss_find_upcall(struct rpc_pipe *pipe, kuid_t uid, struct pid *pid, const struct gss_auth *auth)
{
 ...
 * If an upcall owned by our uid and pid already exists, then we return a reference
...
gss_add_msg(struct gss_upcall_msg *gss_msg)
{
	...
	spin_lock(&pipe->lock);
	old = __gss_find_upcall(pipe, gss_msg->uid, gss_msg->pid, gss_msg->auth);
	if (old == NULL) {
				...
				const struct cred *cred)
{
	...
	len = scnprintf(p, buflen, "mech=%s uid=%d pid=%d lpid=%d",
			mech->gm_name,
			from_kuid_munged(userns, gss_msg->uid),
			pid_nr(gss_msg->pid),
			pid_nr(gss_msg->leading_pid));
...
gss_alloc_msg(struct gss_auth *gss_auth,
	      kuid_t uid, struct pid *pid, const char *service_name)
{
	...
	gss_msg->pid = get_pid(pid);
	gss_msg->leading_pid = get_leading_ns_pid();
	return gss_msg;
...
gss_setup_upcall(struct gss_auth *gss_auth, struct rpc_cred *cred)
{
	...
	gss_new = gss_alloc_msg(gss_auth, uid, gss_cred->gc_pid,
				gss_cred->gc_principal);
	if (IS_ERR(gss_new))
		return gss_new;
...
static struct gss_upcall_msg *
gss_find_downcall(struct rpc_pipe *pipe, kuid_t uid, struct pid *pid)
{
	struct gss_upcall_msg *pos;
	list_for_each_entry(pos, &pipe->in_downcall, list) {
		if (!uid_eq(pos->uid, uid))
		  ...
		  int version)
{
	...
	if (version == 2) {
		p = simple_get_bytes(p, end, &pid_nr, sizeof(pid_nr));
		if (IS_ERR(p)) {
			err = PTR_ERR(p);
			goto err;
		}
		...
		rcu_read_lock();
		pid = get_pid(find_pid_ns(pid_nr, &init_pid_ns));
		rcu_read_unlock();
	}
	...
	spin_lock(&pipe->lock);
	gss_msg = gss_find_downcall(pipe, uid, pid);
	if (gss_msg == NULL) {
		spin_unlock(&pipe->lock);
		goto err_put_ctx;
	}
...
static ssize_t
gss_pipe_downcall_v0(struct file *filp, const char __user *src, size_t mlen)
{
	return gss_pipe_downcall(filp, src, mlen, 0);
}

static ssize_t
gss_pipe_downcall_v1(struct file *filp, const char __user *src, size_t mlen)
{
	return gss_pipe_downcall(filp, src, mlen, 1);
}

static ssize_t
gss_pipe_downcall_v2(struct file *filp, const char __user *src, size_t mlen)
{
	return gss_pipe_downcall(filp, src, mlen, 2);
}
...
static int gss_pipe_open_v2(struct inode *inode)
{
	return gss_pipe_open(inode, 2);
}
...
gss_create_new(const struct rpc_auth_create_args *args, struct rpc_clnt *clnt)
{
	 ...
	 * the new pipe first.
	 */
	gss_pipe = gss_pipe_get(clnt, "gssd-v2", &gss_upcall_ops_v2);
	if (IS_ERR(gss_pipe)) {
		err = PTR_ERR(gss_pipe);
		goto err_destroy_credcache;
	}
	gss_auth->gss_pipe[2] = gss_pipe;

	gss_pipe = gss_pipe_get(clnt, "gssd", &gss_upcall_ops_v1);
	if (IS_ERR(gss_pipe)) {
		err = PTR_ERR(gss_pipe);
		goto err_destroy_pipe_2;
	}
...
err_destroy_pipe_2:
	gss_pipe_free(gss_auth->gss_pipe[2]);
...
static int
gss_hash_cred(struct rpc_auth *auth, struct auth_cred *acred, unsigned int hashbits)
{
	...
	vers = get_pipe_version(net);
	u = hash_64(from_kuid(&init_user_ns, acred->cred->fsuid), hashbits);
	if (vers == 2) {
		u64 p = acred->pid ? hash_64(pid_nr(acred->pid), hashbits) : 0;
		u = hash_64(u + p, hashbits);
	}
	put_pipe_version(net);
...
out:
	if (acred->principal != NULL) {
		...
		ret = uid_eq(rc->cr_cred->fsuid, acred->cred->fsuid)
			&& (gss_cred->gc_pid ?
			    gss_cred->gc_pid == acred->pid : true);
	}
	return ret;
}
...
static const struct rpc_pipe_ops gss_upcall_ops_v2 = {
	.upcall		= gss_v1_upcall,
	.downcall	= gss_pipe_downcall_v2,
	.destroy_msg	= gss_pipe_destroy_msg,
	.open_pipe	= gss_pipe_open_v2,
	.release_pipe	= gss_pipe_release,
};
