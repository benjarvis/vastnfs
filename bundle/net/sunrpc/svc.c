...
static int
param_set_pool_mode(const char *val, MODULE_PARAM_CALL_CONST struct kernel_param *kp)
{
	int *ip = (int *)kp->arg;
...
static int
param_get_pool_mode(char *buf, MODULE_PARAM_CALL_CONST struct kernel_param *kp)
{
	int *ip = (int *)kp->arg;
...
{
#if defined(COMPAT_DETECT_KTHREAD_DATA_EXPORTED)
	struct svc_rqst	*rqstp;
#endif
	struct task_struct *task;
	unsigned int state = serv->sv_nrthreads-1;
...
#if defined(COMPAT_DETECT_PAGEVEC_DROP_6_5)
