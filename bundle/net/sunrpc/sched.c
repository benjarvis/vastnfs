...
{
	static atomic64_t rpc_pid;

	task->tk_pid = atomic64_inc_return(&rpc_pid);
}
#else
...
EXPORT_SYMBOL_GPL(rpc_release_calldata);
...
static void rpc_init_task(struct rpc_task *task, const struct rpc_task_setup *task_setup_data)
{
	...
	task->tk_cred_owner = task_setup_data->owner_pid ?
		get_pid(task_setup_data->owner_pid) :
		get_task_pid(current, PIDTYPE_SID);
	task->tk_owner = current->tgid;
#ifdef CONFIG_NVFS
	task->tk_gpu_idx = task_setup_data->gpu_idx;
	task->tk_gpu_advised = task_setup_data->gpu_advised;
#endif
	task->tk_multipath_hash = task_setup_data->multipath_hash;
...
static void rpc_free_task(struct rpc_task *task)
{
	...
	trace_rpc_task_free(task, NULL);
	put_rpccred(task->tk_op_cred);
	put_pid(task->tk_cred_owner);
	task->tk_cred_owner = NULL;
