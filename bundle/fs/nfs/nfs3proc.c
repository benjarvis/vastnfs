...
#define nfs3_rpc_call_sync_multipath(clnt, msg, flags, hash, tracecode) ({ \
		int _loop_res_; \
		do { \
			_loop_res_ = rpc_call_sync_multipath(clnt, msg, flags, hash, tracecode); \
			if (_loop_res_ != -EJUKEBOX) \
				break; \
			freezable_schedule_timeout_killable_unsafe(NFS_JUKEBOX_RETRY_TIME); \
			_loop_res_ = -ERESTARTSYS; \
		} while (!fatal_signal_pending(current)); \
		_loop_res_; \
	}) \

#define nfs3_rpc_call_sync(clnt, msg, flags, tracecode)	\
	nfs3_rpc_call_sync_multipath(clnt, msg, flags, 0, tracecode)
...
static int
do_proc_get_root(struct nfs_server *server,
		 ...
		 struct nfs_fsinfo *info)
{
	...
	status = nfs3_rpc_call_sync(client, &msg, 0, {
		trace_nfs3proc_fsinfo(_task_, server, fhandle);
	});
	...
	if (status == 0 && !(info->fattr->valid & NFS_ATTR_FATTR)) {
		...
		status = nfs3_rpc_call_sync(client, &msg, 0, {
			trace_nfs3proc_getattr(_task_, server, fhandle);
		});
		   ...
		   struct nfs_fsinfo *info)
{
	...
	status = do_proc_get_root(server, server->client, fhandle, info);
	if (status && server->nfs_client->cl_rpcclient != server->client)
		status = do_proc_get_root(server, server->nfs_client->cl_rpcclient, fhandle, info);
	return status;
}
		...
		struct inode *inode)
{
	...
	status = nfs3_rpc_call_sync_multipath(server->client, &msg, task_flags,
					 NFS_FH_HASH(fhandle, server->client),
					 {});
			...
			struct iattr *sattr)
{
	...
	nfs_fattr_init(fattr);
	status = nfs3_rpc_call_sync_multipath(NFS_CLIENT(inode), &msg, 0,
					      NFS_FH_HASH_INODE(inode), {});
	if (status == 0) {
		   ...
		   unsigned short task_flags)
{
	...
	nfs_fattr_init(fattr);
	status = nfs3_rpc_call_sync_multipath(NFS_CLIENT(dir), &msg, task_flags,
					      NFS_FH_HASH_INODE(dir), {
		trace_nfs3proc_lookup(_task_, NFS_SERVER(dir), NFS_FH(dir));
	});
	...
	if (status >= 0 && !(fattr->valid & NFS_ATTR_FATTR)) {
		...
		status = nfs3_rpc_call_sync_multipath(NFS_CLIENT(dir), &msg, task_flags,
						      NFS_FH_HASH_INODE(dir), {
			trace_nfs3proc_getattr(_task_, NFS_SERVER(dir), fhandle);
		});
	}
...
static int nfs3_proc_access(struct inode *inode, struct nfs_access_entry *entry)
{
	...
	status = nfs3_rpc_call_sync_multipath(NFS_CLIENT(inode), &msg, 0, NFS_FH_HASH_INODE(inode), {
		trace_nfs3proc_access(_task_, NFS_SERVER(inode), NFS_FH(inode));
	});
...
static int nfs3_proc_readlink(struct inode *inode, struct nfs_fh *fh, struct page *page,
...
nfs3_do_create(struct inode *dir, struct dentry *dentry, struct nfs3_createdata *data)
{
	...
	status = nfs3_rpc_call_sync_multipath(NFS_CLIENT(dir), &data->msg, 0, NFS_FH_HASH_INODE(dir), {});
...
nfs3_proc_remove(struct inode *dir, struct dentry *dentry)
{
	...
	status = nfs3_rpc_call_sync_multipath(NFS_CLIENT(dir), &msg, 0, NFS_FH_HASH_INODE(dir), {});
...
nfs3_proc_link(struct inode *inode, struct inode *dir, const struct qstr *name)
{
	...
	status = nfs3_rpc_call_sync_multipath(NFS_CLIENT(inode), &msg, 0, NFS_FH_HASH_INODE(dir), {});
	nfs_post_op_update_inode(dir, res.dir_attr);
			     ...
			     struct nfs_readdir_res *nr_res)
{
	...
	status = nfs3_rpc_call_sync_multipath(NFS_CLIENT(dir), &msg, 0, NFS_FH_HASH_INODE(dir), {
		if (nr_arg->plus)
			trace_nfs3proc_readdirplus(_task_, NFS_SERVER(dir), NFS_FH(dir));
		else
			trace_nfs3proc_readdir(_task_, NFS_SERVER(dir), NFS_FH(dir));
	});
		 ...
		 struct nfs_fsstat *stat)
{
	...
	status = nfs3_rpc_call_sync_multipath(server->client, &msg, 0,
					 NFS_FH_HASH(fhandle, server->client), {});
		   ...
		   struct nfs_pathconf *info)
{
	...
	nfs_fattr_init(info->fattr);
	status = nfs3_rpc_call_sync(server->client, &msg, 0, {});
