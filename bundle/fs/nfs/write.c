...
nfs_page_find_private_request(struct page *page)
{
	...
	if (!PagePrivate(page))
		return NULL;
	spin_lock(compat_address_space_private_lock(mapping));
	req = nfs_page_private_request(page);
	if (req) {
		WARN_ON_ONCE(req->wb_head != req);
		kref_get(&req->wb_kref);
	}
	spin_unlock(compat_address_space_private_lock(mapping));
	return req;
}
...
static void nfs_mapping_set_error(struct page *page, int error)
{
	...
	SetPageError(page);
	/* See ba3a3390c9b17abd71ac44f4a8f7647f3e4e01fc */
#ifdef COMPAT_DETECT_SUPERBLOCK_WB_ERR
...
#if defined(COMPAT_DETECT_WRITE_CACHE_PAGES_6_3)
static int nfs_writepages_callback(struct folio *folio, struct writeback_control *wbc, void *data)
{
       int ret;

       ret = nfs_do_writepage(compat_folio_page(folio), wbc, data);
       if (ret != AOP_WRITEPAGE_ACTIVATE)
               folio_unlock(folio);
       return ret;
}
		...
		unsigned int bytes)
{
	...
	if (mntflags & NFS_MOUNT_NO_EXTEND) {
		/*
		 * Avoid extending only if write != lazy. Offsets are likely
		 * overlapping anyway in the default write=lazy case.
		 */
		if (mntflags & (NFS_MOUNT_WRITE_EAGER | NFS_MOUNT_WRITE_WAIT))
			goto out_flushme;
	}
				...
				struct inode *inode, unsigned int pagelen)
{
	...
	if (unlikely(mntflags & NFS_MOUNT_NO_EXTEND))
		return 0;
			...
			int how, int flags)
{
	...
	struct rpc_task_setup task_setup_data = {
		...
		.multipath_hash = NFS_FH_HASH_INODE_DATA(data->inode) |
			SUNRPC_MULTIPATH_DATA_HASH_MARK,
		.flags = RPC_TASK_ASYNC | flags,
		.priority = priority,
		.owner_pid = get_pid(data->context->pid),
	};
...
static void nfs_commit_done(struct rpc_task *task, void *calldata)
{
	struct nfs_commit_data	*data = calldata;
        ...
        dprintk("NFS: task %llx nfs_commit_done (status %d)\n",
...
#endif
{
	 ...
	 * request */
	req = nfs_lock_and_join_requests(compat_folio_page(folio));
#else
...
#if defined(COMPAT_DETECT_WAIT_ON_PAGE_FSCACHE) && defined(COMPAT_DETECT_PAGE_FOLIO_5_19)
	if (PageFsCache(page)) {
...
int __init nfs_init_writepagecache(void)
{
	 ...
	 * Limit the default to 1024M
	 */
	nfs_congestion_kb = (16*int_sqrt(compat_totalram_pages())) << (PAGE_SHIFT-10);
	if (nfs_congestion_kb > 1024*1024)
		nfs_congestion_kb = 1024*1024;
