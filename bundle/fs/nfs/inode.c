...
#ifndef COMPAT_DETECT_ALLOC_INODE_SB
static inline void *
...
{
	nfs_async_write_flush(NFS_I(inode), 0);
...
static unsigned int nfs_multipath_get_client_idx_by_dentry(struct dentry *dentry)
{
	struct nfs_server *server = NFS_SB(dentry->d_sb);

	if (server->superblock.nr_alt_servers == 0) {
		/* No multipath enabled */
		return 0;
	}

	return 1 + (hash_ptr(dentry, 5) % (1 + server->superblock.nr_alt_servers));
}

struct nfs_server *nfs_multipath_get_client_by_dentry(struct dentry *dentry)
{
	struct nfs_server *server = NFS_SB(dentry->d_sb);
	struct inode *inode;
	unsigned int idx;
	...
	if (inode) {
		/* If inode exists, it is already bound */
		return NFS_SERVER(inode);
	}

	idx = nfs_multipath_get_client_idx_by_dentry(dentry);
	if (idx > 1)
		server = server->superblock.alt_server[idx - 2];
	return server;
}
EXPORT_SYMBOL(nfs_multipath_get_client_by_dentry);
 ...
 */
struct inode *
nfs_fhget_server(struct super_block *sb, struct nfs_fh *fh, struct nfs_fattr *fattr, struct nfs4_label *label,
		 struct nfs_server *server, struct dentry *dentry)
{
	struct nfs_find_desc desc = {
		.fh	= fh,
		.fattr	= fattr
	...
	if (inode->i_state & I_NEW) {
		...
		if (S_ISREG(inode->i_mode)) {
			...
			if (server && NFS_SB(sb)->superblock.nr_alt_servers > 0)
				nfsi->server_idx1 = server->idx + 1;
			else if (dentry)
				nfsi->server_idx1 =
					nfs_multipath_get_client_idx_by_dentry(dentry);
		...
		if (nfsi->server_idx1)
			trace_nfs_inode_bind_server_idx(inode);
...
EXPORT_SYMBOL_GPL(nfs_fhget_server);

#define NFS_VALID_ATTRS (ATTR_MODE|ATTR_UID|ATTR_GID|ATTR_SIZE|ATTR_ATIME|ATTR_ATIME_SET|ATTR_MTIME|ATTR_MTIME_SET|ATTR_FILE|ATTR_OPEN)

int
nfs_setattr(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns) struct dentry *dentry,
	    struct iattr *attr)
{
...
int nfs_getattr(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns) const struct path *path,
		struct kstat *stat, u32 request_mask, unsigned int query_flags)
{
	...
	if ((request_mask & (STATX_CTIME | STATX_MTIME)) &&
	    S_ISREG(inode->i_mode) && !(server->flags & NFS_MOUNT_RELMTIME))
...
out_no_revalidate:
	...
	generic_fillattr(COMPAT_INIT_USER_NS_ARG COMPAT_FILLATTR(request_mask) inode, stat);
...
struct nfs_open_context *alloc_nfs_open_context(struct nfs_server *server,
						...
						struct file *filp)
{
	...
	ctx->pid = get_task_pid(current, PIDTYPE_SID);
	ctx->ll_cred = NULL;
	ctx->state = NULL;
	ctx->mode = f_mode;
	ctx->flags = 0;
...
EXPORT_SYMBOL_GPL(put_nfs_open_context_sync);
...
int nfs_open(struct inode *inode, struct file *filp)
{
	...
	ctx = alloc_nfs_open_context(NFS_SERVER(inode), file_dentry(filp), filp->f_mode, filp);
	if (IS_ERR(ctx))
		return PTR_ERR(ctx);
...
static void init_once(void *foo)
{
	...
	INIT_LIST_HEAD(&nfsi->async_write_list);
	atomic_set(&nfsi->async_write_count, 0);
	spin_lock_init(&nfsi->async_write_lock);
...
#if defined(CONFIG_NFS_MOUNT_TRACES) && \
	defined(COMPAT_DETECT_USER_PATH_AT) && \
	defined(COMPAT_DETECT_DO_MOUNT)
#define NFS_MOUNT_TRACES
#endif

#if defined(__x86_64__) && defined(NFS_MOUNT_TRACES)

#if defined(COMPAT_DETECT_FTRACE_GET_REGS)
static void do_mount_ftrace_handle(unsigned long ip, unsigned long parent_ip,
				   struct ftrace_ops *op,
				   struct ftrace_regs *ftrace_regs)
{
	struct pt_regs *regs = ftrace_get_regs(ftrace_regs);
#else
static void do_mount_ftrace_handle(unsigned long ip, unsigned long parent_ip,
				   struct ftrace_ops *op, struct pt_regs *regs)
{
	...
	struct path path;
	char buf[0x100], *mount_point_str;
	int retval;

	kernel_type = (const char *)regs->dx; /* Third parameter */
	if (!kernel_type || strncmp(kernel_type, "nfs", 3))
		return;

	dir_name = (const char __user *)regs->si; /* Second parameter */
	retval = user_path_at(AT_FDCWD, dir_name, LOOKUP_FOLLOW, &path);
	if (retval < 0)
		return;

	mount_point_str = d_path(&path, buf, sizeof(buf));
	path_put(&path);

	if (IS_ERR(mount_point_str))
		return;

	trace_mount_new(kernel_type, mount_point_str);
}

static bool hook_do_mount_registered = false;

struct ftrace_ops hook_do_mount_ops = {
	.func			= do_mount_ftrace_handle,
	.flags			= FTRACE_OPS_FL_SAVE_REGS,
	.private		= NULL,
};
#endif

static void nfs_debug_init(void)
{
#if defined(__x86_64__) && defined(NFS_MOUNT_TRACES)
	int ret;

	ret = ftrace_set_filter(&hook_do_mount_ops,
				"do_mount", strlen("do_mount"), 1);
	if (ret)
		return;

	ret = register_ftrace_function(&hook_do_mount_ops);
	if (ret)
		return;

	hook_do_mount_registered = true;
#endif
}

static void nfs_debug_exit(void)
{
#if defined(__x86_64__) && defined(NFS_MOUNT_TRACES)
	if (hook_do_mount_registered) {
		unregister_ftrace_function(&hook_do_mount_ops);
		ftrace_set_notrace(&hook_do_mount_ops, NULL, 0, 1);
	}
#endif
}

struct workqueue_struct *nfs_async_write_workqueue __read_mostly;
...
static int __init init_nfs_fs(void)
{
	...
	/*
	 * Background loading of `rpcrdma` as a result of mount sometimes does not
	 * catch up with the mount system-call, failing rpcrdma mounts. This is
	 * a workaround.
	 */
	request_module("rpcrdma");

	wq = alloc_workqueue("nfs_aw", WQ_MEM_RECLAIM | WQ_UNBOUND, 0);
	if (!wq)
		goto out11;
	nfs_async_write_workqueue = wq;
...
out9:
	nfs_sysfs_exit();
out10:
	destroy_workqueue(nfs_async_write_workqueue);
out11:
	return err;
}
...
static void __exit exit_nfs_fs(void)
{
	...
	nfsiod_stop();
	nfs_sysfs_exit();
	nfs_client_cleanup_trids();
	nfs_fs_context_cleanup_trids();
	destroy_workqueue(nfs_async_write_workqueue);
}
