...
struct nfs_client *nfs4_alloc_client(const struct nfs_client_initdata *cl_init)
{
	...
	clp->cl_mig_gen = 1;
	clp->cl_server_idx = cl_init->server_idx;
...
static void nfs4_add_trunk(struct nfs_client *clp, struct nfs_client *old)
{
	...
	struct rpc_clnt_test_and_add_xprt_flags test_and_add_flags = {
		.in_max_connect = &max_connect,
	};
	...
	rpc_clnt_add_xprt(old->cl_rpcclient, &xprt_args,
			  rpc_clnt_test_and_add_xprt, &test_and_add_flags);
}
...
found:
		...
		status = nfs4_proc_setclientid_confirm(pos, &clid, cred, NULL);
		switch (status) {
		...
		struct nfs_fs_context *ctx)
{
	...
	if (ctx) {
		cl_init.local_ports = ctx->local_ports;
		cl_init.no_idle_xprt = ctx->no_idle_xprt;
		cl_init.local_ports_failover = ctx->local_ports_failover;
		cl_init.mdconnect = 0; /* Not relevant for NFSv4.x */
	}
	...
	trace_nfs4_server_bind_nfs_client(server);
...
/*
 * Potentially create a multi-server setup based on specified client
 * parameters.
 */
struct nfs_server *nfs4_create_multi_server(struct fs_context *fc)
{
	struct nfs_fs_context *ctx = nfs_fc2context(fc);
	struct nfs_server *primary = NULL;
	struct nfs_server *sub;
	struct rpc_portgroup *remoteports = ctx->remote_ports;
	char *orig_hostname = ctx->nfs_server.hostname;
	u32 orig_nconnect = ctx->nfs_server.nconnect;
	u32 orig_max_connect = ctx->nfs_server.max_connect;
	struct sockaddr_storage addr;
	bool replace_hostname;
	uint32_t remoteport_offset = 0, divised_per_client;
	uint32_t spray_nconnect = orig_nconnect;
	uint32_t reminder;
	uint32_t idx = 0;
	unsigned short port;
	...
	if (!remoteports || remoteports->nr == 0 || orig_nconnect == 0) {
		primary = nfs4_create_server(fc);
		goto out;
	}
	port = rpc_get_port(&ctx->nfs_server.address);
	reminder = orig_nconnect % remoteports->nr;
	divised_per_client = orig_nconnect / remoteports->nr;

	remoteport_offset = rpc_calc_portgroup_offset(
		fc->net_ns, 0, remoteports,
		ctx->nfs_server.remoteports_offset_provided,
		ctx->nfs_server.remoteports_offset);

	/*
	 * If the provided hostname is an IP address, then show
	 * different IP addresses in /proc/self/net/nfsfs/servers
	 */
	replace_hostname = orig_hostname &&
		rpc_pton(fc->net_ns,
			 orig_hostname, strlen(orig_hostname),
			 (struct sockaddr *)&addr, sizeof(addr)) != 0;

	while (spray_nconnect > 0) {
		char buf[0x100] = {};
		const int i = remoteport_offset;
		const uint32_t client_nconnect = divised_per_client
			+ ((idx < reminder) ? 1 : 0);
		ctx->nfs_server.nconnect = client_nconnect;
		ctx->nfs_server.max_connect = divised_per_client + 1;

		rpc_ntop((struct sockaddr *)&remoteports->addrs[i],
			 buf, sizeof(buf));

		printk(KERN_DEBUG "%s: remoteports[%d] = %s, nconnect=%d\n",
		       __func__, i, buf, client_nconnect);

		if (replace_hostname)
			ctx->nfs_server.hostname = buf;

		ctx->nfs_server.addrlen = remoteports->lens[i];
		memcpy(&ctx->nfs_server.address, &remoteports->addrs[i],
		       remoteports->lens[i]);
		rpc_set_port(&ctx->nfs_server.address, port);
		...
		if (!primary) {
			...
			sub = nfs4_create_server(fc);
			if (!IS_ERR(sub))
				nfs_server_add_alt(primary, sub);
		}

		remoteport_offset++;
		if (remoteport_offset >= remoteports->nr)
			remoteport_offset = 0;

		BUG_ON(spray_nconnect < client_nconnect);
		spray_nconnect -= client_nconnect;
		idx += 1;
	}

	ctx->nfs_server.hostname = orig_hostname;
	ctx->nfs_server.nconnect = orig_nconnect;
	ctx->nfs_server.max_connect = orig_max_connect;
out:
	if (remoteports && remoteports->nr > 0 && !IS_ERR_OR_NULL(primary)) {
		/* pass ownership over remoteports to primary NFS4 server */
		primary->remote_ports = remoteports;
