...
#ifndef __NFS_INTERNAL__
#define __NFS_INTERNAL__
...
#define NFS_MAX_MD_CONNECTIONS  8

#define NFS_SB_MASK (SB_RDONLY|SB_NOSUID|SB_NODEV|SB_NOEXEC|SB_SYNCHRONOUS)

extern const struct export_operations nfs_export_ops;
extern struct workqueue_struct *nfs_async_write_workqueue;
...
struct nfs_client_initdata {
	...
	size_t addrlen;
	struct rpc_portgroup *local_ports;      /* Local ports bind */
	bool local_ports_failover;
	struct rpc_portgroup *remote_ports;     /* Remote ports */
	u32 remote_ports_offset;
	bool remote_ports_offset_provided;
	bool spread_reads;
  	bool spread_writes;
  	bool no_idle_xprt;
	...
	u32 server_idx;				/* NFSv4.1 server multipath index */
	struct net *net;
...
#define nfs_errorf(fc, fmt, ...) \
	({nfs_fs_context_trace("error", fc, fmt, ## __VA_ARGS__); \
	(compat_fs_context_to_log(fc) ?						\
	errorf(fc, fmt, ## __VA_ARGS__) :			\
	({ dprintk(fmt "\n", ## __VA_ARGS__); })); })

#define nfs_ferrorf(fc, fac, fmt, ...) \
	({nfs_fs_context_trace("ferror", fc, fmt, ## __VA_ARGS__); \
	(compat_fs_context_to_log(fc) ?		\
	errorf(fc, fmt, ## __VA_ARGS__) :			\
	({ dfprintk(fac, fmt "\n", ## __VA_ARGS__); })); })

#define nfs_invalf(fc, fmt, ...) \
	({nfs_fs_context_trace("invalf", fc, fmt, ## __VA_ARGS__); \
	(compat_fs_context_to_log(fc) ?		\
	invalf(fc, fmt, ## __VA_ARGS__) :			\
	({ dprintk(fmt "\n", ## __VA_ARGS__);  -EINVAL; })); })

#define nfs_finvalf(fc, fac, fmt, ...) \
	({nfs_fs_context_trace("finvalf", fc, fmt, ## __VA_ARGS__); \
	(compat_fs_context_to_log(fc) ?		\
	invalf(fc, fmt, ## __VA_ARGS__) :			\
	({ dfprintk(fac, fmt "\n", ## __VA_ARGS__);  -EINVAL; })); })

#define nfs_warnf(fc, fmt, ...) \
	({nfs_fs_context_trace("warnf", fc, fmt, ##__VA_ARGS__); \
	(compat_fs_context_to_log(fc) ?		\
	warnf(fc, fmt, ## __VA_ARGS__) :			\
	({ dprintk(fmt "\n", ## __VA_ARGS__); })); })

#define nfs_fwarnf(fc, fac, fmt, ...) \
	({nfs_fs_context_trace("warn", fc, fmt, ## __VA_ARGS__); \
	(compat_fs_context_to_log(fc) ?		\
	warnf(fc, fmt, ## __VA_ARGS__) :			\
	({ dfprintk(fac, fmt "\n", ## __VA_ARGS__); })); })
...
extern struct nfs_server *nfs4_create_multi_server(struct fs_context *);
extern struct nfs_server *nfs4_create_referral_server(struct fs_context *);
extern void nfs_server_add_alt(struct nfs_server *parent, struct nfs_server *srv);
...
COMPAT_FS_CONTEXT_DETECTOR_DECL(nfs_fs_type);
...
int nfs_create(COMPAT_INIT_USER_NS_PARAM struct inode *, struct dentry *,
	       umode_t, bool);
int nfs_mkdir(COMPAT_INIT_USER_NS_PARAM struct inode *, struct dentry *,
	      umode_t);
int nfs_rmdir(struct inode *, struct dentry *);
int nfs_unlink(struct inode *, struct dentry *);
int nfs_symlink(COMPAT_INIT_USER_NS_PARAM struct inode *, struct dentry *,
		const char *);
int nfs_link(struct dentry *, struct inode *, struct dentry *);
int nfs_mknod(COMPAT_INIT_USER_NS_PARAM struct inode *, struct dentry *, umode_t,
	      dev_t);
int nfs_rename(COMPAT_INIT_USER_NS_PARAM struct inode *, struct dentry *,
...
#define nfs_file_splice_read generic_file_splice_read
#else
...
extern uint32_t nfs_metadata_hashing_mask;
extern uint32_t nfs_metadata_client_mask;

/* Use the bits and leave room for SUNRPC_MULTIPATH_DATA_HASH_MARK */
#define NFS_MULTIPATH_HASH(expr) ((((uint16_t)(expr)) & 0x3fff) + 1)

#define NFS_FH_HASH_INODE_DATA(inode) \
	NFS_MULTIPATH_HASH(nfs_fhandle_hash(NFS_FH(inode)))

#define _NFS_FH_CLIENT_HASH(client) \
	((((size_t)(client)) >> 8) & nfs_metadata_client_mask)

#define NFS_FH_HASH(fhandle, client)  \
	NFS_MULTIPATH_HASH(((nfs_fhandle_hash(fhandle) & nfs_metadata_hashing_mask)) + \
	  _NFS_FH_CLIENT_HASH(client))

#define NFS_FH_HASH_INODE(inode)  \
	NFS_MULTIPATH_HASH((nfs_fhandle_hash(NFS_FH(inode)) & nfs_metadata_hashing_mask) + \
	  _NFS_FH_CLIENT_HASH(NFS_CLIENT(inode))) \

extern struct nfs_server *nfs_multipath_get_client_by_dentry(struct dentry *dentry);
