...
{
       nfs_readdir_clear_array(compat_folio_page(folio));
}
#endif
...
bool nfs_use_readdirplus(struct inode *dir, struct dir_context *ctx)
{
	if (!nfs_server_capable(dir, NFS_CAP_READDIRPLUS))
		return false;
	if (NFS_SERVER(dir)->flags & NFS_MOUNT_FORCE_READDIRPLUS)
		return true;
...
again:
	...
	inode = nfs_fhget_server(dentry->d_sb, entry->fh, entry->fattr, entry->label, NULL, dentry);
...
struct dentry *nfs_lookup(struct inode *dir, struct dentry * dentry, unsigned int flags)
{
	...
	inode = nfs_fhget_server(dentry->d_sb, fhandle, fattr, label, NULL, dentry);
	res = ERR_CAST(inode);
	if (IS_ERR(res))
		goto out_label;
...
static struct nfs_open_context *create_nfs_open_context(struct nfs_server *server, struct dentry *dentry, int open_flags, struct file *filp)
{
	return alloc_nfs_open_context(server, dentry, flags_to_mode(open_flags), filp);
}
...
static int nfs_finish_open(struct nfs_open_context *ctx,
			   ...
			   FS_ATOMIC_OPEN_OPENED_PARAM)
{
	int err;

	err = finish_open(file, dentry, do_open FS_ATOMIC_OPEN_OPENED_ARG);
	if (err)
		goto out;
...
int nfs_atomic_open(struct inode *dir, struct dentry *dentry,
		    struct file *file, unsigned open_flags,
		    umode_t mode FS_ATOMIC_OPEN_OPENED_PARAM)
{
	DECLARE_WAIT_QUEUE_HEAD_ONSTACK(wq);
	...
	server = nfs_multipath_get_client_by_dentry(dentry);

retry_open:
	ctx = create_nfs_open_context(server, dentry, open_flags, file);
	err = PTR_ERR(ctx);
	if (IS_ERR(ctx))
		goto out;
	...
	if (!IS_ERR(inode) && NFS_SERVER(inode) != server) {
		/*
		 * In NFSv4.1 client-based multipath, it is possible inode
		 * instantiated with a different client. We must use this
		 * client for all stateful operations.
		 */
	...
	*opened = created;
	...
	err = nfs_finish_open(ctx, ctx->dentry, file, open_flags FS_ATOMIC_OPEN_OPENED_ARG);
	trace_nfs_atomic_open_exit(dir, ctx, open_flags, err);
	put_nfs_open_context(ctx);
out:
 ...
 */
int nfs_create(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns) struct inode *dir,
 ...
 */
int
nfs_mknod(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns) struct inode *dir,
 ...
 * See comments for nfs_proc_create regarding failed operations.
 */
int nfs_mkdir(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns) struct inode *dir,
 ...
 */
int nfs_symlink(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns) struct inode *dir,
 ...
 * the rename.
 */
int nfs_rename(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns) struct inode *old_dir,
...
int nfs_permission(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns)
		   struct inode *inode,
		   int mask)
{
...
out_notsup:
	...
	if (res == 0)
		res = generic_permission(COMPAT_INIT_USER_NS_ARG inode, mask);
	goto out;
}
