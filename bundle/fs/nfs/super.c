...
#ifdef COMPAT_DETECT_FREE_INODE
...
{
	return COMPAT_REGISTER_FILESYSTEM(nfs4_fs_type);
}

static void unregister_nfs4_fs(void)
{
	COMPAT_UNREGISTER_FILESYSTEM(nfs4_fs_type);
}
#else
static int __init register_nfs4_fs(void)
{
...
int __init register_nfs_fs(void)
{
	int ret;
        ...
        ret = COMPAT_REGISTER_FILESYSTEM(nfs_fs_type);
	if (ret < 0)
		goto error_0;
...
error_2:
	unregister_nfs4_fs();
error_1:
	COMPAT_UNREGISTER_FILESYSTEM(nfs_fs_type);
error_0:
	return ret;
}
...
int nfs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	trace_nfs_statfs_exit(inode, 0);

	return 0;

 out_err:
	dprintk("%s: statfs error = %d\n", __func__, -error);
	trace_nfs_statfs_exit(inode, error);
	return error;
}
...
static void nfs_show_nconnect_options(struct seq_file *m, struct nfs_server *nfss)
{
	struct nfs_client *clp = nfss->nfs_client;
	int nconnect = clp->cl_nconnect;
	int i;

	for (i = 0; i < nfss->superblock.nr_alt_servers; i++) {
		struct nfs_server *server = nfss->superblock.alt_server[i];
		if (!server)
			continue;
		nconnect += server->nfs_client->cl_nconnect;
	}

	if (nconnect > 0)
		seq_printf(m, ",nconnect=%u", nconnect);
}

int nfs_get_succeeding_ipaddr(const struct sockaddr *addr,
					struct sockaddr_storage *buff)
{
	...
	case AF_INET: 
		in = (struct sockaddr_in *)buff;
		memcpy(in, addr, sizeof(*in));

		in->sin_addr.s_addr = htonl(ntohl(in->sin_addr.s_addr) + 1);
		retval = sizeof(*in);
		break;
	case AF_INET6:
		in6 = (struct sockaddr_in6 *)buff;
		memcpy(in6, addr, sizeof(*in6));

		if (!(in6->sin6_addr.s6_addr32[3] =
		htonl(ntohl(in6->sin6_addr.s6_addr32[3]) + 1))) /* Overflow */
			in6->sin6_addr.s6_addr32[2] = 
				htonl(ntohl(in6->sin6_addr.s6_addr32[2]) + 1);
		retval = sizeof(*in6);
		break;
	}
	return retval;
...
static bool nfs_is_succeeding_addr(const struct sockaddr *prev,
					const struct sockaddr *addr)
{
	if (prev && prev->sa_family == addr->sa_family) {
		struct sockaddr_storage buff;
		if (nfs_get_succeeding_ipaddr(prev, &buff) > 0)
			return rpc_cmp_addr((struct sockaddr *)&buff, addr);
	}
	return false;
}
...
{
	char buff[64];
	if (!addr || rpc_ntop(addr, buff, sizeof(buff)) <= 0)
		seq_printf(m, "%c", delim);
	else
		seq_printf(m, "%c%s", delim, buff);
}

static void nfs_show_portgroup(struct seq_file *m, const char *param_name,
			       const struct rpc_portgroup *portgroup)
{
	if (!param_name || !portgroup)
		return;

	seq_printf(m, ",%s", param_name);
	if (portgroup->dns) {
		seq_puts(m, "=dns");
	} else {
		char delim = '=';
		struct sockaddr *prev = NULL;
		int i, nr = min_t(int, portgroup->nr, RPC_MAX_PORTS);

		for (i = 0; i < nr; ++i) {
			struct sockaddr* addr =
				(struct sockaddr *)(portgroup->addrs + i);
			if (nfs_is_succeeding_addr(prev, addr)) {
				delim = '-';
			} else {
				if (prev && delim == '-') {
					nfs_seq_put_ntop(m, delim, prev);
					delim = '~';
				}
				nfs_seq_put_ntop(m, delim, addr);
				delim = '~';
			}
			prev = addr;
		}
				   ...
				   int showdefaults)
{
	static const struct proc_nfs_info {
		...
		{ NFS_MOUNT_OPT_LOCK_FLUSH, ",optlockflush", "" },
		{ NFS_MOUNT_RELMTIME, ",relmtime", "" },
		{ NFS_MOUNT_NO_EXTEND, ",noextend", "" },
		{ 0, NULL, NULL }
	};
	...
	rcu_read_unlock();
	nfs_show_nconnect_options(m, nfss);
	if (version == 4) {
	...
	if (nfss->flags & NFS_MOUNT_NO_EXTEND)
		seq_puts(m, ",noextend");
	if (nfss->flags & NFS_MOUNT_OPT_LOCK_FLUSH)
		seq_puts(m, ",optlockflush");
	...
	nfs_show_portgroup(m, "localports", clp->cl_localports);
	if (clp->cl_localports_failover)
		seq_puts(m, ",localports_failover");

	nfs_show_portgroup(m, "remoteports", nfss->remote_ports ?
				nfss->remote_ports : clp->cl_remoteports);
	if (clp->cl_remoteports_offset_provided)
		seq_printf(m, ",remoteports_offset=%u", clp->cl_remoteports_offset);

	if (clp->cl_no_idle_xprt)
                seq_puts(m, ",noidlexprt");
	if (clp->cl_mdconnect)
                seq_printf(m, ",mdconnect=%d", clp->cl_mdconnect);
	if (clp->cl_spread_reads)
		seq_puts(m, ",spread_reads");
	if (clp->cl_spread_writes)
		seq_puts(m, ",spread_writes");
}
...
int nfs_show_stats(struct seq_file *m, struct dentry *root)
{
	...
	rpc_clnt_show_stats(m, nfss->client, false);
	...
	if (sunrpc_expose_id_in_mountstats) {
		...
		for (i = 0; i < nfss->superblock.nr_alt_servers; i++) {
			struct nfs_server *server = nfss->superblock.alt_server[i];
			if (found == 0)
				seq_printf(m, "\talt-sunrpc-id:");

			found++;
			seq_printf(m, "\t%x", server->client->cl_clid);
		}
...
static void nfs_fill_super(struct super_block *sb, struct nfs_fs_context *ctx)
{
	...
	case 2:
		sb->s_time_gran = 1000;
#if defined(COMPAT_DETECT_FS_TIMESTAMP_RANGES)
		sb->s_time_min = 0;
...
static int nfs_compare_super(struct super_block *sb, struct fs_context *fc)
{
	...
	if (!nfs_compare_super_address(old, server))
		return 0;
	if (old->nfs_client->cl_xprt_id != server->nfs_client->cl_xprt_id)
		return 0;
	...
	if (!nfs_compare_userns(old, server))
		return 0;
#ifdef COMPAT_DETECT_SECURITY_SB_MNT_OPTS_COMPAT
...
static void nfs_set_readahead(struct backing_dev_info *bdi,
			      struct nfs_server *server)
{
#if defined(VM_READAHEAD_PAGES)
	bdi->ra_pages = VM_READAHEAD_PAGES;
	bdi->io_pages = server->rpages;
#else
	/* Before commit - c128e575514ce9: NFS: Optimise the default readahead size */
#define NFS_MAX_READAHEAD      (RPC_DEF_SLOT_TABLE - 1)
	bdi->ra_pages = server->rpages * NFS_MAX_READAHEAD;
#endif
}
...
int nfs_get_tree_common(struct fs_context *fc)
{
	...
	struct nfs_server *server = ctx->server, *alt_server;
	int error;
...
#if defined(COMPAT_DETECT_FS_CONTEXT_LSM_FLAGS)
	...
	if (s->s_fs_info != server) {
		...
		if (s->s_bdi->ra_pages == 0) {
			/*
			 * Prior to Linux commit 55b2598e84e97efc5d952958cb5e34236c43276b
			 * the block layer used to set these defaults.
			 */
			nfs_set_readahead(s->s_bdi, server);
		} else {
			s->s_bdi->io_pages = server->rpages;
		}

		trace_nfs_fs_context_bind_nfs_server(ctx, server);

		server->super = s;
		for (i = 0; i < server->superblock.nr_alt_servers; i++) {
			alt_server = server->superblock.alt_server[i];
			alt_server->super = s;
			alt_server->s_dev = server->s_dev;
		}
	}
