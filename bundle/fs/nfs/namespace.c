...
static int
nfs_namespace_getattr(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns)
		      const struct path *path, struct kstat *stat,
		      u32 request_mask, unsigned int query_flags)
{
	if (NFS_FH(d_inode(path->dentry))->size != 0)
		return nfs_getattr(COMPAT_INIT_USER_NS_WRAP(mnt_userns) path, stat, request_mask,
				   query_flags);
	generic_fillattr(COMPAT_INIT_USER_NS_ARG COMPAT_FILLATTR(request_mask) d_inode(path->dentry), stat);
	return 0;
}

static int
nfs_namespace_setattr(COMPAT_INIT_USER_NS_PARAM_N(mnt_userns) struct dentry *dentry,
		      struct iattr *attr)
{
	if (NFS_FH(d_inode(dentry))->size != 0)
		return nfs_setattr(COMPAT_INIT_USER_NS_WRAP(mnt_userns) dentry, attr);
	return -EACCES;
}
