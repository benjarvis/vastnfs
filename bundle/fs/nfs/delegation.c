...
again:
	...
	list_for_each_entry_rcu(ctx, &nfsi->open_files, list) {
		...
		seq = compat_raw_seqcount_begin(&sp->so_reclaim_seqcount);
 ...
 *
 */
void nfs_inode_reclaim_delegation(struct nfs_server *server,
				  ...
				  unsigned long pagemod_limit)
{
	...
	if (delegation != NULL) {
		...
		rcu_read_unlock();
		nfs_inode_set_delegation(server, inode, cred, type, stateid,
					 pagemod_limit);
	}
}
...
static int nfs_do_return_delegation(struct inode *inode, struct nfs_delegation *delegation, int issync)
{
	...
	if (!test_bit(NFS_DELEGATION_REVOKED, &delegation->flags)) {
		...
		spin_unlock(&delegation->lock);
		res = nfs4_proc_delegreturn(delegation->server, inode, cred,
				&delegation->stateid,
				issync);
		put_cred(cred);
	}
...
static struct nfs_delegation *nfs_detach_delegation(struct nfs_inode *nfsi,
		struct nfs_delegation *delegation)
{
	struct nfs_client *clp = delegation->server->nfs_client;
...
nfs_inode_detach_delegation(struct inode *inode)
{
	...
	if (delegation != NULL)
		delegation = nfs_detach_delegation(nfsi, delegation);
	rcu_read_unlock();
	return delegation;
}
 ...
 */
int nfs_inode_set_delegation(struct nfs_server *server,
			     struct inode *inode, const struct cred *cred,
			     fmode_t type,
...
int nfs4_inode_make_writeable(struct inode *inode)
{
	...
	delegation = nfs4_get_valid_delegation(inode);
	if (delegation == NULL ||
	    (nfs4_has_session(delegation->server->nfs_client) &&
	     (delegation->type & FMODE_WRITE))) {
		rcu_read_unlock();
		return 0;
	}
...
restart_locked:
	list_for_each_entry_rcu(delegation, &server->delegations, super_list) {
		...
		rcu_read_unlock();
		if (delegation != NULL) {
			if (nfs_detach_delegation(NFS_I(inode), delegation) != NULL)
				nfs_free_delegation(delegation);
