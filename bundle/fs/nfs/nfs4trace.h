...
DECLARE_EVENT_CLASS(nfs4_clientid_event,
		...
		TP_STRUCT__entry(
			...
			__field(u64, clientid)
			__field(unsigned long, error)
		),

		TP_fast_assign(
			__entry->error = error < 0 ? -error : 0;
			__entry->clientid = clp->cl_clientid;
			__assign_str(dstaddr, clp->cl_hostname);
		),

		TP_printk(
			"error=%ld (%s) dstaddr=%s clientid=0x%016llx",
			-__entry->error,
			show_nfsv4_errors(__entry->error),
			__get_str(dstaddr),
			(unsigned long long)__entry->clientid
		)
);
...
DECLARE_EVENT_CLASS(nfs4_inode_stateid_event,
		...
		TP_printk(
			"error=%ld (%s) fileid=%02x:%02x:%llu fhandle=0x%08x server_idx1=%d "
			"stateid=%d:0x%08x",
			-__entry->error,
			show_nfsv4_errors(__entry->error),
			MAJOR(__entry->dev), MINOR(__entry->dev),
...
TRACE_EVENT(nfs4_server_bind_nfs_client,
	TP_PROTO(
...
DECLARE_EVENT_CLASS(nfs4_write_event,
		...
		TP_printk(
			"error=%ld (%s) fileid=%02x:%02x:%llu fhandle=0x%08x "
			"server_idx1=%d offset=%lld count=%u res=%u stateid=%d:0x%08x "
			"layoutstateid=%d:0x%08x",
			-__entry->error,
			show_nfsv4_errors(__entry->error),
			MAJOR(__entry->dev), MINOR(__entry->dev),
