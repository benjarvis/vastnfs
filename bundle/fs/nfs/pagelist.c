		      ...
		      const struct rpc_call_ops *call_ops, int how, int flags)
{
	...
	struct rpc_task_setup task_setup_data = {
		...
		.multipath_hash = (((clnt->cl_spread_reads && hdr->rw_mode == FMODE_READ) ||
				    (clnt->cl_spread_writes && hdr->rw_mode == FMODE_WRITE)) ?
				   0 : NFS_FH_HASH_INODE_DATA(hdr->inode))
				  | SUNRPC_MULTIPATH_DATA_HASH_MARK,
		.owner_pid = get_pid(nfs_req_openctx(hdr->req)->pid),
...
#ifdef CONFIG_NVFS
	if (hdr->args.count > 0 && hdr->args.pages && hdr->args.pages[0])
		task_setup_data.gpu_advised =
			rpc_page_gpu_idx(hdr->args.pages[0],
				   &task_setup_data.gpu_idx);
#endif
