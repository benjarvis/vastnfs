...
TRACE_EVENT(nfsd_dirent,
	...
	TP_STRUCT__entry(
		__field(u32, fh_hash)
		__field(u64, ino)
		COMPAT___STRING_LEN(name, name, namlen)
	),
	TP_fast_assign(
		__entry->fh_hash = fhp ? knfsd_fh_hash(&fhp->fh_handle) : 0;
		__entry->ino = ino;
		COMPAT__ASSIGN_STR_LEN(name, name, namlen)
	),
...
DECLARE_EVENT_CLASS(nfsd_clid_class,
	...
	TP_STRUCT__entry(
		...
		COMPAT___STRING_LEN(name, name, clp->cl_name.len)
	),
	TP_fast_assign(
		__entry->cl_boot = clp->cl_clientid.cl_boot;
		__entry->cl_id = clp->cl_clientid.cl_id;
		...
		memcpy(__entry->verifier, (void *)&clp->cl_verifier,
		       NFS4_VERIFIER_SIZE);
		COMPAT__ASSIGN_STR_LEN(name, clp->cl_name.data, clp->cl_name.len);
	),
...
TRACE_EVENT(nfsd_file_acquire,
	...
	TP_printk("xid=0x%x hash=0x%x inode=%p may_flags=%s status=%u",
			__entry->xid, __entry->hash, __entry->inode,
			show_nfsd_may_flags(__entry->may_flags),
			__entry->status)
);
