...
for flavor in %flavors_to_build; do
	...
	export KVER=%{kernel_release $K_BUILD}
	cd $PWD/obj/$flavor
	mkdir -p %{buildroot}/usr/share/vastnfs
	cp build-info.txt %{buildroot}/usr/share/vastnfs
	mkdir -p %{buildroot}/usr/bin
	cp scripts/vastnfs-ctl %{buildroot}/usr/bin
...
%if 0%{?rhel} >= 8
# Protocol from: https://git.centos.org/rpms/kmod-kvdo/blob/c8/f/SPECS/kmod-kvdo.spec
%post

# Remember the modules we just installed so that we can process them in the posttrans
modules=( $(find /lib/modules/%{KVERSION}/%{install_mod_dir}/ -name \*.ko ) )
...
# Run weak-modules at the end of the transaction
...
# Save the the pathnames of installed kernel modules before we remove them
rpm -ql vastnfs-%{_version}-%{_release1}%{?_dist} | grep '\.ko$' > /var/run/rpm-kmod-vastnfs-modules

%postun

# Run weak-modules on the modules we are removing
modules=( $(cat /var/run/rpm-kmod-vastnfs-modules) )
rm /var/run/rpm-kmod-vastnfs-modules
...
%defattr(-,root,root,-)
/usr/bin/vastnfs-ctl
/usr/share/vastnfs/build-info.txt
